...
Функция ПодготовитьТаблицуКВыгрузке(ЗНАЧ Товары,ОбСклад)
...
Процедура ККМOfflineПриАктивизацииСтроки(Элемент)
	...
	ККМOffline	=Элемент.ТекущаяСтрока.Значение.ID;
...
Процедура ККМOfflineВыбор(Элемент, ЭлементСписка)
	...
	Иначе
		...
		ТоварыВыгрузка=ПодготовитьТаблицуКВыгрузке(Товары,Склад);
		
		Состояние("Выгрузка в кассу");
		Результат = ПолучитьСерверТО().ВыгрузитьТоварыККМ(ККМOffline, ТоварыВыгрузка,ТестовыйРежим);
...
//ZAM+++ 22.05.13 #1046904 Сделать загрузку товаров во все кассы по одной кнопке
	...
	Для Каждого Элемент Из СпККМOffline Цикл
		...
		Товары=ПодготовитьТаблицуКВыгрузке(ТоварыКВыгрузке,Элемент.Значение.Склад);
