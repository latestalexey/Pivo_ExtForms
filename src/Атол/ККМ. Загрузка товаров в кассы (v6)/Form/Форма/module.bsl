...
Функция ПолучитьТаблицуТоваровДляВыгрузки()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |				И (КомплектующиеНоменклатуры.Комплектующая В ИЕРАРХИИ (&ГрПивоРозлив))
	               |	ГДЕ
	...
	Запрос.УстановитьПараметр("Бонус3",				ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоКоду("000000070"));
...
Функция ПодготовитьТаблицуКВыгрузке(ЗНАЧ Товары,ОбСклад,КассаККМ)
	...
	Бонус3=ИТИИндустрияОбщийМодульКлиентСервер.ПринадлежитКатегории(КассаККМ,Справочники.КатегорииОбъектов.НайтиПоКоду("000000175"));
...
Функция РасчетБонусаПоИерархии(Знач Товар)
	...
	Отбор=Новый Структура("Объект,Свойство",,ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоКоду("000000070"));
...
Процедура АвтоВыгрузка() Экспорт
