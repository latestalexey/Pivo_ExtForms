...
Функция СтрокаВыгрузкиТовара(ЗНАЧ СтрокаТаблицыТоваров,Наименование,НаименованиеЭкран,ШтрихКоды, 
								...
								Кратность,ПЛУ,ВыгружатьГруппыПечати,Объем=0,СкидкаЗаЗаказССайта=ЛОЖЬ);  //+svetl 18.11.15
	...
	+ ?(СкидкаЗаЗаказССайта,Формат(СтрокаТаблицыТоваров.Цена*0.9,"ЧДЦ=2;ЧРД='.'"),"")		+	мРазделитель	// Серия (20)     //svetl делаем скидку 10% для работы скрипта у сотрудников колл-центра. Ночных цен на пиццерии нет.
...
Функция ВыгрузитьТовары(Объект, Товары, ЧастичнаяВыгрузка = Ложь) Экспорт
	...
	ГрПиццаВайнера                  =Справочники.Номенклатура.НайтиПоКоду("00000012293").Ссылка;   //+svetl 18.11.15
	ГрПиццаПиццерия                  =Справочники.Номенклатура.НайтиПоКоду("00000028898").Ссылка;   //+svetl 18.11.15
	...
	Для Каждого Товар Из Товары Цикл
		...
		//+svetl 18.11.15 down
		СкидкаЗаЗаказССайта = Ложь;                                        
		Если Товар.Номенклатура.ПринадлежитЭлементу(ГрПиццаВайнера) ИЛИ Товар.Номенклатура.ПринадлежитЭлементу(ГрПиццаПиццерия) Тогда 
			 СкидкаЗаЗаказССайта = Истина;
		КонецЕсли;
		//+svetl 18.11.15 up
		...
		Строка=СтрокаВыгрузкиТовара(	Товар, _Наименование, _НаименованиеЭкран,
										...
										_Кратность,_ПЛУ,ВыгружатьГруппыПечати,Товар.Объем, СкидкаЗаЗаказССайта);  //+svetl 18.11.15
		Файл.ДобавитьСтроку(Строка);
	КонецЦикла;
...
Функция ЗагрузитьОтчет__(Объект, Отчет, ТабОтмененныхЧеков=Неопределено) Экспорт
