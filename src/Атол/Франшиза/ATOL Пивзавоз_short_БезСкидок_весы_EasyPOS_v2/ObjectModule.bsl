...
Функция ВыгрузитьТовары(Объект, Товары, ЧастичнаяВыгрузка = Ложь) Экспорт
	...
	Для Каждого Товар Из Товары Цикл
		...
		Пока Кат.Следующий() Цикл
			Если Кат.Категория = Справочники.КатегорииОбъектов.НайтиПоНаименованию("Без скидки(франшиза)",Истина) Тогда    //Не применять скидку в POS
				СоСкидкой = Ложь;
			КонецЕсли;
		КонецЦикла;
		...
		Если Товар.Номенклатура.ПринадлежитЭлементу(Справочники.номенклатура.НайтиПоКоду("00000000221")) Тогда 
			СоСкидкой = Ложь;   //делаем скидку на бутылки чтобы на НЕ них распространялись бонусы
		КонецЕсли;
		//+Svetl up
