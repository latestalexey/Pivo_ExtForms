...
Функция ВыгрузитьТовары(Объект, Товары, ЧастичнаяВыгрузка = Ложь) Экспорт
	...
	Для Каждого Товар Из Товары Цикл
		...
		Если ШтрихКоды.Количество()= 1 Тогда
			...
			+ ",1,1,0,1,0," + ?(Товар.Цена = 0 ИЛИ ЗначениеЗаполнено(Товар.ЦенаНочь), "1", "0") + ",1"   + мРазделитель
			...
			+ ?(ЗначениеЗаполнено(Товар.ЦенаНочь),Формат(Товар.ЦенаНочь,"ЧДЦ=2"),"")	   + мРазделитель // Серия (20)
			+ ""                                                   					+ мРазделитель // Сертификат (21)
			+ ""                                                   					+ мРазделитель // Код ККМ (22)
			+ ""                                                   					+ мРазделитель // Код налоговой группы (23)
			+ ""                                                   					+ мРазделитель // Код весов с ПЭ (24)
			+ ?(Товар.ВесовойТовар,Формат(Товар.ПЛУ,"ЧН=0; ЧГ=0"), "")              + мРазделитель // Код товара в весах ПЭ (25)
		...
		Иначе
			...
			+ ""                                                   					+ мРазделитель // Код налоговой группы
			+ ""                                                   					+ мРазделитель // Код весов с ПЭ
			+ ?(Товар.ВесовойТовар,Формат(Товар.ПЛУ,"ЧН=0; ЧГ=0"), "")              + мРазделитель // Код товара в весах ПЭ
...
//Str-						
						...
						Если КодВидаОплаты = 1 Тогда
							...
							Врем.ОплаченоВидомОплаты1 = Врем.ОплаченоВидомОплаты1 + Сумма - СуммаСдачи;     //Нал
						ИначеЕсли КодВидаОплаты = 2 Тогда
							//Сообщить("Безнал:" + НомерЧека + "-" + ДатаТранзакции + "=" + Сумма);
							Врем.ОплаченоВидомОплаты2 = Врем.ОплаченоВидомОплаты2 + Сумма - СуммаСдачи;     //Безнал
						Иначе
							//Сообщить("Доставка:" + НомерЧека + "-" + ДатаТранзакции + "=" + Сумма);
							Врем.ОплаченоВидомОплаты3 = Врем.ОплаченоВидомОплаты3 + Сумма - СуммаСдачи;     //Доставка
						КонецЕсли;
...
//						Сообщить("Скидка на чек");
						...
						ИнформацияПоСкидкеСтр = СтрПолучитьСтроку(ТекущаяСтрока, 8);
						...
						Попытка
							...
							ИнформацияПоСкидке = Строка(ИнформацияПоСкидкеСтр);  //Номер карты
...
//							Сообщить("Автоматическая;");
							...
							Врем.ИнформацияПоСкидке = ИнформацияПоСкидке;
