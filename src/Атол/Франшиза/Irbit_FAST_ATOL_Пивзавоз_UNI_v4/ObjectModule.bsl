...
Функция СтрокаВыгрузкиТовара(ЗНАЧ СтрокаТаблицыТоваров,Наименование,НаименованиеЭкран,ШтрихКоды, 
								Сертификат,ГТД,СоСкидкой,ПоНулевойЦенеПриНаличииКарты,
								Кратность,ПЛУ,ВыгружатьГруппыПечати,Объем=0)
	...
	+ ?(СоСкидкой=Неопределено,"",?(СоСкидкой,"999998","999999"))							+	мРазделитель	// Код родительской группы (16)
	...
	//KAV++ Крапивин Андрей. 20.02.2015 10:07:24 # .
	+ ?(СоСкидкой=Неопределено,"",?(СоСкидкой,"","0,00"))									+	мРазделитель	// Максимальная скидка, % (29)
	//KAV-- Крапивин Андрей. 20.02.2015 10:07:27
	...
	//KAV++ Крапивин Андрей. 20.02.2015 12:07:06
	+?(ВыгружатьГруппыПечати,мРазделитель+"1"+мРазделитель+""+мРазделитель+"2","");								//Группы печати. Пока только для пиццерии (39)=1,(40)=,(41)=2
	Возврат Строка;
КонецФункции
...
Функция ВыгрузитьТовары(Объект, Товары, ЧастичнаяВыгрузка = Ложь) Экспорт
    ...
    КассаККМ=РегистрыСведений.ТорговоеОборудование.Получить(Новый Структура("Идентификатор",Объект.Идентификатор)).Модель;
	
	ИсклФиксБонус			= ИТИИндустрияОбщийМодульКлиентСервер.ПринадлежитКатегории(КассаККМ,Справочники.КатегорииОбъектов.НайтиПоНаименованию("Исключить фикс. бонус"));
	НеВыгружатьСерт			= ИТИИндустрияОбщийМодульКлиентСервер.ПринадлежитКатегории(КассаККМ,Справочники.КатегорииОбъектов.НайтиПоНаименованию("Не выгружать подарочные сертификаты"));
	НеВыгружатьИерархию		= ИТИИндустрияОбщийМодульКлиентСервер.ПринадлежитКатегории(КассаККМ,Справочники.КатегорииОбъектов.НайтиПоНаименованию("Без иерархии"));
	ВыгружатьГруппыПечати	= ИТИИндустрияОбщийМодульКлиентСервер.ПринадлежитКатегории(КассаККМ,Справочники.КатегорииОбъектов.НайтиПоНаименованию("Выгружать группы печати"));
	НуженДопПользователь	= ИТИИндустрияОбщийМодульКлиентСервер.ПринадлежитКатегории(КассаККМ,Справочники.КатегорииОбъектов.НайтиПоНаименованию("Оператор Call-центра"));
	Результат = мНетОшибки;
	...
	//KAV++ Крапивин Андрей. 20.02.2015 10:03:56
	Если Не НеВыгружатьИерархию Тогда
		...
		//Str-
	КонецЕсли;
	//KAV-- Крапивин Андрей. 20.02.2015 10:04:03
	...
	Для Каждого Товар Из Товары Цикл
		...
		#Если НаКлиенте Тогда
			Состояние("Выгрузка товара "+Строка(Сч)+" из "+Строка(ВсегоТоваров));
		#КонецЕсли
		...
		Если мОптимизированнаяВыгрузка Тогда
			...
			ИначеЕсли Не ИсклФиксБонус Тогда
		...
		//Сертификат=?(Товар.Номенклатура.ПринадлежитЭлементу(ГрПивоПоЛитрам) ИЛИ Товар.Номенклатура.ПринадлежитЭлементу(ГрПивоРазливное),"15%",Сертификат);   //+svetl  повышенные бонусы
		
		//Сертификат=?(
		// СокрЛП(Товар.Номенклатура.Код) = "00000000083"
		//ИЛИ СокрЛП(Товар.Номенклатура.Код) = "00000032216"
		//ИЛИ СокрЛП(Товар.Номенклатура.Код) = "00000028818"
		//ИЛИ СокрЛП(Товар.Номенклатура.Код) = "00000028819"
		//ИЛИ СокрЛП(Товар.Номенклатура.Код) = "00000028820"
		//ИЛИ СокрЛП(Товар.Номенклатура.Код) = "00000028821"
		//ИЛИ СокрЛП(Товар.Номенклатура.Код) = "00000028822"
		//ИЛИ СокрЛП(Товар.Номенклатура.Код) = "00000011475"
		//ИЛИ СокрЛП(Товар.Номенклатура.Код) = "00000002485"
		//ИЛИ СокрЛП(Товар.Номенклатура.Код) = "00000032239"
		//ИЛИ СокрЛП(Товар.Номенклатура.Код) = "00000029093"
		//ИЛИ СокрЛП(Товар.Номенклатура.Код) = "00000029095"
		//ИЛИ СокрЛП(Товар.Номенклатура.Код) = "00000029097"
		//ИЛИ СокрЛП(Товар.Номенклатура.Код) = "00000029099"
		//ИЛИ СокрЛП(Товар.Номенклатура.Код) = "00000029102"
		//	,"23%",Сертификат);   //+svetl  повышенные бонусы

		
		// ///ГРУППА 1
		//		
		//Сертификат=?(
		//Товар.Номенклатура.Код = "00000032979"
		//ИЛИ Товар.Номенклатура.Код = "00000032980"
		//ИЛИ Товар.Номенклатура.Код = "00000032981"
		//ИЛИ Товар.Номенклатура.Код = "00000032982"
		//ИЛИ Товар.Номенклатура.Код = "00000032983"
		//ИЛИ Товар.Номенклатура.Код = "00000000083"
		//ИЛИ Товар.Номенклатура.Код = "00000032216"
		//ИЛИ Товар.Номенклатура.Код = "00000028818"
		//ИЛИ Товар.Номенклатура.Код = "00000028819"
		//ИЛИ Товар.Номенклатура.Код = "00000028820"
		//ИЛИ Товар.Номенклатура.Код = "00000028821"
		//ИЛИ Товар.Номенклатура.Код = "00000028822"
		//ИЛИ Товар.Номенклатура.Код = "00000011802"
		//ИЛИ Товар.Номенклатура.Код = "00000032233"
		//ИЛИ Товар.Номенклатура.Код = "00000029015"
		//ИЛИ Товар.Номенклатура.Код = "00000029017"
		//ИЛИ Товар.Номенклатура.Код = "00000029018"
		//ИЛИ Товар.Номенклатура.Код = "00000029019"
		//ИЛИ Товар.Номенклатура.Код = "00000029020"
		//,"30%",Сертификат);   //+svetl  повышенные бонусы
		...
		Строка=СтрокаВыгрузкиТовара(	Товар, _Наименование, _НаименованиеЭкран,
										ШтрихКоды, 
										Сертификат,ГТД,?(НеВыгружатьИерархию,Неопределено,СоСкидкой),ПоНулевойЦенеПриНаличииКарты,
										_Кратность,_ПЛУ,ВыгружатьГруппыПечати,Товар.Объем);
		Файл.ДобавитьСтроку(Строка);
	КонецЦикла;
	
	//KAV++ Добавляем подарочные карты!
	#Если НаКлиенте Тогда
		Состояние("Выгрузка сертификатов");
	#КонецЕсли
	Если НЕ НеВыгружатьСерт Тогда
	...
	//KAV++ Крапивин Андрей. 26.02.2015 14:14:44
	Если НуженДопПользователь Тогда
		//+svetl добавили пользователя Колл-центр. нужно только для пиццерии по сути
		//CALL - ЦЕНТР
		Строка = Формат(4, "ЧЦ=3; ЧДЦ=0; ЧН=0; ЧГ=0")					+ мРазделитель	// 1 Целое Код
		+ ПодготовитьСтроку("Колл центр")								+ мРазделитель	// 2 Строка 100 Наименование)
		+ ПодготовитьСтроку("Колл центр")								+ мРазделитель	// Строка 100 Текст для чека
		+ "5"															+ мРазделитель	// 4 Целое Код профиля пользователя
		+ ПодготовитьСтроку("1735")										+ мРазделитель	// 5 Строка 10 Пароль
		+ ПодготовитьСтроку("1735");													// 6 Строка 255 Карта пользователя
		Файл.ДобавитьСтроку(Строка);
	КонецЕсли;
	//KAV-- Крапивин Андрей. 26.02.2015 14:14:50
...
//  <ПеречислениеСсылка.ТООшибки*> - Результат работы функции.
//
Функция ЗагрузитьОтчет_old(Объект, Отчет) Экспорт
	...
	Позиции.Колонки.Добавить("Скидка");
	...
	Если Файл.Существует() Тогда
		...
		Если Результат = мНетОшибки Тогда
			...
			Если ТекущаяСтрока = "#" Тогда
				...
				ТекОплачено2=Новый Массив; //svetl-skat
				...
				Пока Не ПустаяСтрока(ТекущаяСтрока) Цикл
					...
					ИначеЕсли ТипТранзакции = 11 Тогда
						...
						//KAV++ Крапивин Андрей. 25.03.2015 9:43:10 Теперь сумма оригинальная и отдельно регистрируем цену.
						...
						//KAV-- Крапивин Андрей. 25.03.2015 9:43:46
					...
					ИначеЕсли ТипТранзакции = 15 Тогда //Скидка суммой на позицию товара по цене из справочника/
						...
						Исключение
							Сообщить("Не удолось прочитать скидку для чека "+НомерЧека);
						КонецПопытки;
						Товар.Скидка	= Товар.Скидка+Число(СуммаСкидки);
...
//Str-						
						...
						ИначеЕсли КодВидаОплаты = 2 Тогда
							...
							Если НомерСмены>ТекОплачено2.ВГраница() Тогда
								ТекОплачено2.Вставить(НомерСмены,Сумма - СуммаСдачи); //svetl-skat
							Иначе
								ТекОплачено2[НомерСмены]=ТекОплачено2[НомерСмены]+Сумма - СуммаСдачи; //svetl-skat
							КонецЕсли;
	...
	Если Результат = мНетОшибки Тогда
		...
		Для Каждого Чек Из Врем Цикл
			...
			Для Каждого Позиция Из ПозицииЧека Цикл
				...
				РезПозиция.Скидка		= Позиция.Скидка;
//Str
...
//Str
		...
		ТекСмена=0;
		Для Каждого Док Из Отчет Цикл
			Если ТекСмена<>Док.НомерСмены Тогда
				Если Док.НомерСмены<=ТекОплачено2.ВГраница() Тогда
					Док.ОплаченоВидомОплаты2=ТекОплачено2[Док.НомерСмены];
				Иначе
					Док.ОплаченоВидомОплаты2=0;
				КонецЕсли;
				ТекСмена=Док.НомерСмены;
			КонецЕсли;
		КонецЦикла;
	Иначе
		Сообщить("" + Результат);
