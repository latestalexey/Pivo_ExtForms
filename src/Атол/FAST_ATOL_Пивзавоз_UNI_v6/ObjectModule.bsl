...
Перем ИсклФиксБонус;
Перем НеВыгружатьСерт;
Перем НеВыгружатьИерархию;
Перем ВыгружатьГруппыПечати;
Перем НуженДопПользователь;
Перем ЭтоПиццерия;
Перем НеДобиватьНулями;
...
Функция СтрокаВыгрузкиГруппы(НаименованиеГруппы,КодГруппы)
	...
	+ ?(ЭтоПиццерия,"","0")										+ мРазделитель		// Остаток
...
Функция СтрокаВыгрузкиТовара(ЗНАЧ СтрокаТаблицыТоваров,Наименование,НаименованиеЭкран,ШтрихКоды, 
								...
								Кратность,ПЛУ,ВыгружатьГруппыПечати,Объем=0,СписаниеОстатков="0",СкидкаЗаЗаказССайта=Ложь)
								
	ШтрихКод = ?(ШтрихКоды.Количество() > 0, "", ",");
	Для Каждого СтрокаМассива Из ШтрихКоды Цикл
		Если НеДобиватьНулями Тогда
	...
	//Определим Доп цену
	Если ЭтоПиццерия Тогда
		Если СкидкаЗаЗаказССайта Тогда
			ДопЦена=Формат(СтрокаТаблицыТоваров.Цена*0.9,"ЧДЦ=2;ЧРД='.'")
		Иначе
			ДопЦена="";
		КонецЕсли;
	Иначе
		Если ЗначениеЗаполнено(СтрокаТаблицыТоваров.ЦенаНочь) Тогда
			ДопЦена=Формат(СтрокаТаблицыТоваров.ЦенаНочь,"ЧДЦ=2;ЧРД='.'");
		Иначе
			ДопЦена="";
		КонецЕсли;
	КонецЕсли;
	...
	+ ?(СписаниеОстатков="1",Формат(СтрокаТаблицыТоваров.Остаток, "ЧЦ=17; ЧДЦ=3; ЧН=0; ЧГ=0"),"")						+	мРазделитель	// Остаток (6)
	...
	// • редактирование цены
	// • ввод количества вручную
	//+ ?(ПЛУ="", 0, 1)+",1,1,0,1,0," + ?(СтрокаТаблицыТоваров.Цена = 0 ИЛИ СтрокаТаблицыТоваров.ЦенаНочь<>0	// Флаги через запятую: (8) 
	+ ?(ПЛУ="", 0, 1)+",1,1,0,1,"+СписаниеОстатков +","+ ?(СтрокаТаблицыТоваров.Цена = 0 ИЛИ СтрокаТаблицыТоваров.ЦенаНочь<>0
	...
	+ ?(Объем=0 ,"",Формат(Объем,"ЧДЦ=2; ЧРД=.; ЧГ=")) 										+	мРазделитель	//  (36)
	+ ""																					+	мРазделитель	//  (37)
...
Функция ВыгрузитьТовары(Объект, Товары, ЧастичнаяВыгрузка = Ложь) Экспорт
	...
	ЭтоПиццерия				= ИТИИндустрияОбщийМодульКлиентСервер.ПринадлежитКатегории(КассаККМ,Справочники.КатегорииОбъектов.НайтиПоНаименованию("Это пиццерия"));
	НеДобиватьНулями		= ИТИИндустрияОбщийМодульКлиентСервер.ПринадлежитКатегории(КассаККМ,Справочники.КатегорииОбъектов.НайтиПоНаименованию("Не добивать штрихкод нолями"));
    ...
    КатБезСкидки					=Справочники.КатегорииОбъектов.НайтиПоНаименованию("Без скидки",Истина).Ссылка; 
	...
	//+svetl
	ГрПиццаВайнера					=Справочники.Номенклатура.НайтиПоКоду("00000012293").Ссылка;
	ГрПиццаПиццерия					=Справочники.Номенклатура.НайтиПоКоду("00000028898").Ссылка;
	...
	Для Каждого Товар Из Товары Цикл
		...
		КонецЕсли;
		//+svetl up
		СкидкиЗаЗаказССайта=Товар.Номенклатура.ПринадлежитЭлементу(ГрПиццаВайнера) или Товар.Номенклатура.ПринадлежитЭлементу(ГрПиццаВайнера);
		...
		Строка=СтрокаВыгрузкиТовара(	Товар, _Наименование, _НаименованиеЭкран,
										...
										_Кратность,_ПЛУ,ВыгружатьГруппыПечати,Товар.Объем,СписаниеОстатков,СкидкиЗаЗаказССайта);
		Файл.ДобавитьСтроку(Строка);
	КонецЦикла;
...
//ZAM--- 09.09.2015
	...
	//KAV++ Крапивин Андрей. 23.09.2015 16:48:10 #1191280. Дополнительная типизация отчетов
	Отчет.Колонки.Добавить("ТипОтчета"); //0 - стандартный документ, 1 - списание товаров, 2 - на будущее возврат товаров
	Отчет.Колонки.Добавить("ДополнительноеОписание"); //содержит дополнительный комментарий из выгрузки
	//KAV-- Крапивин Андрей. 23.09.2015 16:58:21
	...
	//KAV++ Крапивин Андрей. 23.09.2015 16:48:10 #1191280. Дополнительная типизация отчетов
	Чеки2.Колонки.Добавить("ТипОтчета"); //0 - стандартный документ, 1 - списание товаров, 2 - на будущее возврат товаров
	Чеки2.Колонки.Добавить("ДополнительноеОписание"); //содержит дополнительный комментарий из выгрузки
	//KAV-- Крапивин Андрей. 23.09.2015 16:58:21
		...
		Если Результат = мНетОшибки Тогда
			...
			Если ТекущаяСтрока = "#" Тогда
				...
				Пока Не ПустаяСтрока(ТекущаяСтрока) Цикл
					...
					ТипДокумента		= СтрПолучитьСтроку(ТекущаяСтрока, 13);
...
//							Прервать;
					...
					ИначеЕсли ТипТранзакции = 42 Тогда
						...
						//KAV++ Крапивин Андрей. 23.09.2015 17:01:26 #1191280. Проверяем содержимое дополнительного поля
						Врем.ТипОтчета	= ?(ТипДокумента="6",1,0);
						//KAV++ Крапивин Андрей. 23.09.2015 17:12:47 #1191280. В поле 34 какие-то комментарии
						Врем.ДополнительноеОписание=СтрПолучитьСтроку(ТекущаяСтрока, 34);;
						//KAV-- Крапивин Андрей. 23.09.2015 17:20:43
...
//				РезПозиция.НомерДокумента       = Чек.НомерЧека;  //ТАК КАК НОМЕРОВ В ККМ ВСЕГО 100000 Ненавижу штрих. ;-(
				...
				//KAV++ Крапивин Андрей. 23.09.2015 17:00:05 #1191280. Дополнительный признак вида документа
				РезПозиция.ТипОтчета			= Чек.ТипОтчета;
				РезПозиция.ДополнительноеОписание= Чек.ДополнительноеОписание;
				//KAV-- Крапивин Андрей. 23.09.2015 17:00:54
//Str
			КонецЦикла;
		КонецЦикла;
