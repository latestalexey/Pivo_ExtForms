...
Функция СтрокаВыгрузкиТовара(ЗНАЧ СтрокаТаблицыТоваров,Наименование,НаименованиеЭкран,ШтрихКоды, 
								...
								Кратность,ПЛУ,ВыгружатьГруппыПечати,Объем=0,СкидкаЗаЗаказССайта=ЛОЖЬ)
	...
	+ ?(СкидкаЗаЗаказССайта,Формат(СтрокаТаблицыТоваров.Цена*0.9,"ЧДЦ=2;ЧРД='.'"),"")		+	мРазделитель	// Описание (32)  //+svetl 18.11.15 делаем скидку 10% для работы скрипта у сотрудников колл-центра. Ночных цен на пиццерии нет.
...
Функция ВыгрузитьТовары(Объект, Товары, ЧастичнаяВыгрузка = Ложь) Экспорт
	...
	Для Каждого Товар Из Товары Цикл
		...
		Строка=СтрокаВыгрузкиТовара(	Товар, _Наименование, _НаименованиеЭкран,
										...
										_Кратность,_ПЛУ,ВыгружатьГруппыПечати,Товар.Объем,СкидкаЗаЗаказССайта);          //+svetl 18.11.15 добавлен параметр  СкидкаЗаЗаказССайта
		Файл.ДобавитьСтроку(Строка);
	КонецЦикла;
