...
Процедура ВыгрузитьРеализациюТоваровУслуг(ОбъектXML, СписокДок, ВыводитьИнформационныесообщения)
	...
	|	ВЫРАЗИТЬ(РеализацияТоваровУслуг.Номенклатура.НаименованиеПолное КАК СТРОКА(200)) КАК НаименованиеПолное,
	...
	|	ВЫРАЗИТЬ(РеализацияТоваровУслуг.СерияНоменклатуры.Комментарий КАК СТРОКА(200)) КАК СерияНоменклатурыКомментарий,
	...
	|	НоменклатураКонтрагентов.КодНоменклатурыКонтрагента КАК Ид,
	|	РегистрШтрихКоды.Штрихкод КАК ШтрихКод,
	|	ВЫРАЗИТЬ(РеализацияТоваровУслуг.Ссылка.Комментарий КАК СТРОКА(200)) КАК Комментарий,
	...
	|	РегистрШтрихКоды.ЕдиницаИзмерения КАК ЕдиницаИзмеренияШтрихКод,
	|	РегистрШтрихКоды.ЕдиницаИзмерения.Коэффициент КАК ЕдиницаИзмеренияШтрихКодКоэффициент,
	|	РегистрШтрихКоды.ЕдиницаИзмерения.Наименование КАК ЕдиницаИзмеренияШтрихКодНаименование,
	|	РегистрШтрихКоды.ЕдиницаИзмерения.Вес КАК ЕдиницаИзмеренияШтрихКодВес,
	|	РегистрШтрихКоды.ЕдиницаИзмерения.Объем КАК ЕдиницаИзмеренияШтрихКодОбъем
	|ИЗ
	...
	|	МАКСИМУМ(Дата),
	|	МАКСИМУМ(Валюта),
	|	МАКСИМУМ(КонтрагентСсылка),
	|	МАКСИМУМ(ОрганизацияСсылка),
	...
	|	МАКСИМУМ(СерияНоменклатурыНаименование),
	|	МАКСИМУМ(СерияНоменклатурыСерийныйНомер),
	|	МАКСИМУМ(СерияНоменклатурыСертификат),
	|	МАКСИМУМ(СерияНоменклатурыСрокГодности),
	|	МАКСИМУМ(СерияНоменклатурыНомерГТД),
	|	МАКСИМУМ(СерияНоменклатурыСтранаПроисхожденияКОД),
	|	МАКСИМУМ(СерияНоменклатурыСтранаПроисхожденияНаименование),
	|	МАКСИМУМ(СерияНоменклатурыСтранаПроисхожденияНаименованиеПолное),
	|	МАКСИМУМ(СерияНоменклатурыСтранаПроисхожденияКодАльфа2),
	|	МАКСИМУМ(СерияНоменклатурыКомментарий),
	|	МАКСИМУМ(Сумма),
	|	МАКСИМУМ(СуммаНДС),
	|	МАКСИМУМ(СтавкаНДС),
	...
	|	МАКСИМУМ(ЕдиницаИзмеренияШтрихКод),
	|	МАКСИМУМ(ЕдиницаИзмеренияШтрихКодКоэффициент),
	|	МАКСИМУМ(ЕдиницаИзмеренияШтрихКодНаименование),
	|	МАКСИМУМ(ЕдиницаИзмеренияШтрихКодВес),
	|	МАКСИМУМ(ЕдиницаИзмеренияШтрихКодОбъем)
	|ПО
	|	ДокСсылка,
	|	НоменклатураСсылка");
	...
	РезультатЗапрос = Запрос.Выполнить();
	ВыборкаДок      = РезультатЗапрос.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам); //Документы
	
	Пока ВыборкаДок.СледующийПоЗначениюПоля("ДокСсылка") Цикл
		
		ОбъектXML.ЗаписатьНачалоЭлемента("Документ");
		
		ВыгрузитьЕдиныеРеквизитыДокументов(ОбъектXML, ВыборкаДок, "Отпуск товара", Перечисления.ВидыДоговоровКонтрагентов.СПокупателем, ЛОЖЬ);
		
		ВыгрузитьИнформациюОНалогахДляВыборки(ОбъектXML, ВыборкаДок);
					
		ЗаписатьТекстовойУзел(ОбъектXML, "Комментарий",  ВыборкаДок.Комментарий);
		
		ИдКаталога = ВыборкаДок.ОрганизацияСсылка.УникальныйИдентификатор();
		
		ОбъектXML.ЗаписатьНачалоЭлемента("Товары");
		СохрНомерСтроки = 0;
		Выборка=ВыборкаДок.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам); //Номенклатура
		Пока Выборка.Следующий() Цикл
			...
			//Андрей
			//Выгрузка штрих кодов ВСЕХ
			ВыборкаШтрихкодов=Выборка.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
			ОбъектXML.ЗаписатьНачалоЭлемента("ШтрихкодыТовара");
			ВыгрузитьИнформациюОШтрихкодах(ОбъектXML, ВыборкаШтрихкодов);
			ОбъектXML.ЗаписатьКонецЭлемента();
			//
...
//Андрей. Выгрузка штрихкодов
Процедура ВыгрузитьИнформациюОШтрихкодах(ОбъектXML, Выборка)
	Пока Выборка.Следующий() Цикл
		Если Выборка.ЕдиницаИзмеренияШтрихКодНаименование=Null Тогда Продолжить; КонецЕсли;
		ОбъектXML.ЗаписатьНачалоЭлемента("ШтрихКод");
			ОбъектXML.ЗаписатьАтрибут("Владелец",Выборка.Наименование);
			ОбъектXML.ЗаписатьАтрибут("ЕдиницаИзмеренияШтрихКодНаименование",Выборка.ЕдиницаИзмеренияШтрихКодНаименование);
			ОбъектXML.ЗаписатьАтрибут("ШтрихКодТовара",Выборка.ШтрихКод);
		ОбъектXML.ЗаписатьКонецЭлемента();
	КонецЦикла;                          
КонецПроцедуры
