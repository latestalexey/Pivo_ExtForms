...
Процедура ЗаполнитьСтруктурыЗагрузки() Экспорт
	...
	//ШтрихКоды
	СтруктураЗагрузки.Вставить("ШтрихКод_ШтрихкодыТовара_Товар_Товары_Документ_КоммерческаяИнформация_", "ШтрихКодыТоваров");
...
Процедура ЗаполнитьСоответствияШтрихкодов() Экспорт
	СоответствиеШтрихкодов.Очистить();
	Запрос=Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
		...
		Запрос.УстановитьПараметр("СписокШтрихКодов", Штрихкоды.ВыгрузитьКолонку("Штрихкод"));
		Выборка = Запрос.Выполнить().Выбрать();
	Для Каждого СтрокаТЧ Из Штрихкоды Цикл
		СтрокаТаб=СоответствиеШтрихкодов.Добавить();
		СтрокаТаб.Наименование=СтрокаТЧ.Наименование;
		СтрокаТаб.ЕдиницаИзмеренияФайл=СтрокаТЧ.ЕдиницаИзмеренияНаименование;
		СтрокаТаб.ШтрихКодФайл=СтрокаТЧ.Штрихкод;
	КонецЦикла;
	
	Пока Выборка.Следующий() Цикл
		СтрокаЗаполнить=СоответствиеШтрихкодов.Найти(Выборка.ШтрихКод);
		Если СтрокаЗаполнить<>Неопределено Тогда
			СтрокаЗаполнить.Владелец			= Выборка.Номенклатура;
			СтрокаЗаполнить.ЕдиницаИзмерения	= Выборка.ЕдиницаИзмерения;
			СтрокаЗаполнить.ШтрихКод			= Выборка.ШтрихКод;
		КонецЕсли;
	КонецЦикла;
	Для Каждого СтрокаТЧ Из СоответствиеШтрихкодов Цикл
		Если НЕ ЗначениеЗаполнено(СтрокаТЧ.Владелец) Тогда
			СтрокаТЧ.Владелец=Справочники.Номенклатура.НайтиПоНаименованию(СокрЛП(СтрокаТЧ.Наименование),Истина);
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(СтрокаТЧ.ЕдиницаИзмерения) И ЗначениеЗаполнено(СтрокаТЧ.Владелец) Тогда
			СтрокаТЧ.ЕдиницаИзмерения=Справочники.ЕдиницыИзмерения.НайтиПоНаименованию(СокрЛП(СтрокаТЧ.ЕдиницаИзмеренияФайл),Истина,,СтрокаТЧ.Владелец);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура ЗаменитьТоварыПоСоответствиям() Экспорт
	...
	КонецЦикла;
	//Обновим штрихкоды
	ЗаполнитьСоответствияШтрихкодов();
...
Процедура ОбработатьЭлемент(ОбъектXML, Ключ)
	...
	//Андрей. Загрузка штрихкодов
	ИначеЕсли ЗначениеТипаОбработки = "ШтрихКодыТоваров" Тогда
    	СтрокаШтрихКодов=Штрихкоды.Добавить();
		СтрокаШтрихКодов.ЕдиницаИзмеренияНаименование	= ОбъектXML.ПолучитьАтрибут("ЕдиницаИзмеренияШтрихКодНаименование");
		СтрокаШтрихКодов.Наименование					= ОбъектXML.ПолучитьАтрибут("Владелец");
		СтрокаШтрихКодов.Штрихкод						= ОбъектXML.ПолучитьАтрибут("ШтрихКодТовара");
	КонецЕсли;
...
Функция ТестированиеДанных(СтрокаСообщенияПользователю) Экспорт
	...
	//Андрей
	ЕстьНеопределенныеШтрихКода   = ЛОЖЬ;
	...
	//Андрей. Отдельно обработаем штрихкода
	Для Каждого СтрокаТЧ Из СоответствиеШтрихкодов Цикл
		ЕстьНеопределенныеШтрихКода = ЕстьНеопределенныеШтрихКода ИЛИ Не ЗначениеЗаполнено(СтрокаТЧ.ШтрихКод);
	КонецЦикла;
	Если НЕ мИспользоватьХарактеристикиУНоменклатуры Тогда
		ЕстьНеопределенныеХаракретистикиНоменклатур  = ЛОЖЬ;
	КонецЕсли;
	...
	Если ЕстьНесоответствия Тогда
		...
		КонецЕсли;
		//Андрей
		Если ЕстьНеопределенныеШтрихКода Тогда
			СтрокаСообщенияПользователю = СтрокаСообщенияПользователю + Символы.ПС + "Штрихкоды номенклатуры";
		КонецЕсли;
...
Функция СохранитьПоступлениеТоваровУслуг(ИнформацияДокумента, СтрокиДокумента)
	...
	//Андрей. Сохраним входящий номер дату
	ДокПоступлениеТоваровУслуг.НомерВходящегоДокумента=ИнформацияДокумента.Номер;
	ДокПоступлениеТоваровУслуг.ДатаВходящегоДокумента=ИнформацияДокумента.Дата;
	//Заполним значения по умолчанию
	ТекПользователь = ПараметрыСеанса.ТекущийПользователь;
	ДокПоступлениеТоваровУслуг.СкладОрдер=УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(ТекПользователь, "ОсновнойСклад");
	
	ДокПоступлениеТоваровУслуг.ОтражатьВУправленческомУчете=УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(ТекПользователь, "ОтражатьДокументыВУправленческомУчете");
	ДокПоступлениеТоваровУслуг.ОтражатьВБухгалтерскомУчете=УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(ТекПользователь, "ОтражатьДокументыВБухгалтерскомУчете");
	ДокПоступлениеТоваровУслуг.ОтражатьВНалоговомУчете=УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(ТекПользователь, "ОтражатьДокументыВНалоговомУчете");
	//Если загружаем сегодня то дата актуальная
	Если НачалоДня(ТекущаяДата())=НачалоДня(ИнформацияДокумента.Дата) Тогда
		ДокПоступлениеТоваровУслуг.Дата=ТекущаяДата();
	Иначе
		ДокПоступлениеТоваровУслуг.Дата=НачалоДня(ИнформацияДокумента.Дата);
	КонецЕсли;
