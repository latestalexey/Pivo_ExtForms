...
//
Процедура вОбработатьОбъект(Объект, ПорядковыйНомерОбъекта, ТекстАлгоритма)
...
Функция вВыполнитьОбработку() Экспорт
	...
	Для Индекс = 0 По НайденныеОбъекты.Количество() - 1 Цикл
		...
		вОбработатьОбъект(Объект, Индекс, ТекстАлгоритма);
	КонецЦикла;
	Возврат Индекс;
...
Процедура вЗагрузитьНастройку() Экспорт
	...
	ЭлементыФормы.ТекстПроизвольногоАлгоритма.УстановитьТекст(ТекстПроизвольногоАлгоритма);
