...
Функция СтрокаВыгрузкиТовара(ЗНАЧ Товар,Наименование,НаименованиеЭкран,ШтрихКоды, 
								Сертификат,ГТД,СоСкидкой,ПоНулевойЦенеПриНаличииКарты,
								Кратность,ПЛУ,ВыгружатьГруппыПечати,СтрокаФлагов="0,1,1,0,1,0,0,1",СписаниеОстатков, НеДобиватьНулями, НулевойОстаток)
	...
	+ Формат(?(ЗначениеЗаполнено(Товар.Цена),Товар.Цена,0), "ЧЦ=15; ЧДЦ=2; ЧН=0; ЧГ=0")		+	мРазделитель	// Цена (5)
	+ ?(НулевойОстаток,"0",?(СписаниеОстатков="1",Формат(Товар.Остаток, "ЧЦ=17; ЧДЦ=3; ЧН=0; ЧГ=0"),""))+	мРазделитель	// Остаток (6)
	+ "0"																					+	мРазделитель	// Схема внутренней автоматической скидки (7)
	+ СтрокаФлагов																			+	мРазделитель	// Флаги через запятую: (8) 
	...
	+ Формат(Товар.ЕдиницаИзмерения.Коэффициент,"ЧЦ=7; ЧДЦ=3; ЧН=0; ЧГ=0")					+	мРазделитель	// Коэффициент (14)
	...
	+ ""																					+	мРазделитель	//  (34)
	+ ?(Товар.Объем=0 или Товар.Объем=1,"","1")												+	мРазделитель	//  (35)
	+ ?(Товар.Объем=0 ,"",Формат(Товар.Объем,"ЧДЦ=2; ЧРД=.; ЧГ="))							+	мРазделитель	//  (36)
	+ ""																					+	мРазделитель	//  (37)
...
//  <ПеречислениеСсылка.ТООшибки*> - Результат работы функции.
//
Функция ВыгрузитьТовары(Объект, Товары, УстановкаНовыхСчетчиков = Ложь) Экспорт
	...
	Если УстановкаНовыхСчетчиков = Ложь Тогда
	...
	Для Каждого Товар Из Товары Цикл
		...
		ИначеЕсли Товар.КонтролироватьОстаток Тогда
			СписаниеОстатков="1";
		...
		_НаименованиеЭкран=?(Товар.ГрСигареты ,Товар.Номенклатура.Наименование,Товар.Номенклатура.НаименованиеПолное);
		
		//KAV++ Крапивин Андрей. 10.03.2016 10:44:42 Подготовка строки флагов
		СтрокаФлагов = 
		?(_ПЛУ="", "0", "1")					+","+	// • дробное количество (весовой) 
		"1"										+","+	// • продажа
		"1"										+","+	// • возврат
		?(Товар.КонтролироватьОстаток,"0","1")				+","+	// • отрицательные остатки
		"1"										+","+	// • без ввода количества
		?(Товар.КонтролироватьОстаток,"1","0")				+","+	// • списание остатков
		?(Товар.Цена = 0 
		ИЛИ 
		Товар.ЦенаНочь<>0 
		ИЛИ 
		ПоНулевойЦенеПриНаличииКарты, "1", "0")	+","+	// • редактирование цены
		"1";											// • ввод количества вручную
		Строка=СтрокаВыгрузкиТовара(	Товар, _Наименование, _НаименованиеЭкран,
										ШтрихКоды, 
										Сертификат,
										ГТД,
										?(НеВыгружатьИерархию,Неопределено,СоСкидкой),
										ПоНулевойЦенеПриНаличииКарты,
										_Кратность,
										_ПЛУ,
										ВыгружатьГруппыПечати,
										СтрокаФлагов,
										СписаниеОстатков,
										НеДобиватьНулями,
										НЕ УстановкаНовыхСчетчиков);
		Файл.ДобавитьСтроку(Строка);
	КонецЦикла;
...
//ZAM--- 09.09.2015
	...
	Отчет.Колонки.Добавить("ТипОтчета"); //0 - стандартный документ, 1 - списание товаров, 2 - внесение денег в кассу, 3 - списание денег из кассы, 
	...
	//KAV++ Крапивин Андрей. 23.09.2015 16:48:10 #1191280. Дополнительная типизация отчетов
	Чеки2.Колонки.Добавить("ТипОтчета"); //0 - стандартный документ, 1 - списание товаров, 2 - внесение денег в кассу, 3 - списание денег из кассы, 
...
//							Прервать;
					...
					ИначеЕсли ТипТранзакции = 42 Тогда
						...
						Если ТипДокумента="6" Тогда
							ДопАналитика = СтрПолучитьСтроку(ТекущаяСтрока, 23);
							Если ДопАналитика="12" Тогда
								Врем.ТипОтчета	= 2;
							ИначеЕсли ДопАналитика="13" Тогда
								Врем.ТипОтчета	= 3;
							Иначе
								Врем.ТипОтчета	= 1;
							КонецЕсли;
