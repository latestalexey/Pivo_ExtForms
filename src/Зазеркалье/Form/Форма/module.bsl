...
функция ТекстЗапросаПоОрганизациям()
	...
	Запрос.Текст = 
		...
		|				И Субконто1 В (&СпНоменклатуры)
		|				И (Подразделение = &Подразделение
		|					ИЛИ &Подразделение = ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка))) КАК ОстаткиИОбороты
		|
		|СГРУППИРОВАТЬ ПО
		|	ОстаткиИОбороты.Субконто1,
		|	ОстаткиИОбороты.Организация";
	ПостроительНоменклатура.Выполнить();
	МассНоменклатуры=ПостроительНоменклатура.Результат.Выгрузить().ВыгрузитьКолонку("Ссылка");
	Запрос.УстановитьПараметр("СпНоменклатуры",МассНоменклатуры);
...
Процедура ПриОткрытии()
	...
	ПостроительНоменклатура.Текст="ВЫБРАТЬ
	                              |	СправочникНоменклатура.Ссылка
	                              |ИЗ
	                              |	РегистрСведений.ВидНоменклатуры КАК РегистрВидНоменклатуры
	                              |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
	                              |		ПО РегистрВидНоменклатуры.Номенклатура = СправочникНоменклатура.Ссылка
	                              |{ГДЕ
	                              |	СправочникНоменклатура.Ссылка.* КАК Номенклатура,
	                              |	СправочникНоменклатура.НоменклатурнаяГруппа.*,
	                              |	РегистрВидНоменклатуры.ВидНоменклатуры.*}";
	ПостроительНоменклатура.Отбор.Добавить("Номенклатура");
	ПостроительНоменклатура.Отбор.Добавить("НоменклатурнаяГруппа");
	ПостроительНоменклатура.Отбор.Добавить("ВидНоменклатуры");
...
Процедура КоманднаяПанель3остатки(Кнопка)
	...
	Для каждого строка из таблицаБУ цикл
		...
		Если  (строка.КоличествоКонечныйОстатокДт -строка.КоличествоКонечныйОстатокКт)>0  тогда
			...
			строкаТаблицаТоваровБУ.Дата				= КонецПериода;
		КонецЕсли;
	КонецЦикла;	                                          
...
Процедура Кнопка11Нажатие(Элемент)
	...
	для каждого строка из Реализации1 цикл
		Если строка.взять тогда
			...
			ЗаполнениеДокументовПоУмолчанию.ЗаполнитьСчетаПоУмолчанию(новоепоступление);
			...
			//новоепоступление.СчетУчетаРасчетовСКонтрагентом = СчетаУчета.СчетРасчетовСПоставщиком;
			//новоепоступление.СчетУчетаРасчетовПоАвансам = СчетаУчета.СчетАвансовСПоставщиком;
			//новоепоступление.Склад=склад;
			//Попробуем по грузополучателю определить склад/подразделение
			НужныйСклад=Склад;
			Если ЗначениеЗаполнено(Реализация.Грузополучатель) тогда
				Отбор=Новый Структура("Объект,Свойство",Реализация.Грузополучатель,мСвойствоСклад);
				...
				Если Не ЗначениеЗаполнено(СкладКонтрагента) Тогда
					Ответ=Вопрос("У контрагента "+реализация.Грузополучатель+" не указан связный склад. Выбрать склад?",РежимДиалогаВопрос.ДаНет,30);
					Если Ответ=КодВозвратаДиалога.Да Тогда
						СкладКонтрагента=Справочники.Склады.ПолучитьФормуВыбора().ОткрытьМодально();
						Если ЗначениеЗаполнено(СкладКонтрагента) Тогда
							НужныйСклад=СкладКонтрагента;
							Запись=РегистрыСведений.ЗначенияСвойствОбъектов.СоздатьМенеджерЗаписи();
							Запись.Объект=Реализация.Грузополучатель;
							Запись.Свойство=мСвойствоСклад;
							Запись.Значение=СкладКонтрагента;
							Запись.Записать();
						КонецЕсли;
					КонецЕсли;
				иначе
				...
				КонецЕсли;
			КонецЕсли;
			новоепоступление.Склад=НужныйСклад;
			новоепоступление.ПодразделениеОрганизации=НужныйСклад.ПодразделениеОрганизации;
			для каждого строкатовара из строка.документы.товары цикл
				...
				новаястрокапоступления.СерияНоменклатуры=Справочники.СерииНоменклатуры.ПустаяСсылка();
				// Выполнить общие действия для всех документов при изменении номенклатуры.
				...
				КонецЕсли;
			КонецЦикла;	
		новоепоступление.ЗаполнитьСчетаУчетаВТабЧасти(новоепоступление.Товары,"Товары",истина,истина);
