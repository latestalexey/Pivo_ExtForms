...
Процедура ФлажокОтклАвтоРасчетПриИзменении(Элемент)
	...
	УстановитьДоступностьПолейРаздел1();
	УстановитьДоступностьПолейРаздел2();
...
Процедура ЗаполнитьАвто() Экспорт 
	...
	ПрерватьАвтоЗаполнение = РегламентированнаяОтчетность.ПрерватьАвтоЗаполнениеПоДаннымИБ(ЭтаФорма, "Раздел1,Раздел2");
	...
	ИначеЕсли ИДКонфигурации = "БПКОРП" Тогда
		...
		СписокОрганизаций.Добавить(Организация.Ссылка, Организация.Наименование);
		ЗаполнитьАвтоБухгалтерияКОРП(СписокОрганизаций);
	...
	ИначеЕсли ИДКонфигурации = "УПП" Тогда
		СписокОрганизаций.Добавить(Организация.Ссылка, Организация.Наименование);
		ЗаполнитьАвтоУПП(СписокОрганизаций);
...
Процедура ЗаполнитьАвтоБухгалтерия(СписокОрганизаций)
	...
	|	СведенияОбАлкогольнойПродукции.ВидПродукции.Наименование КАК П000010000101,
	|	СведенияОбАлкогольнойПродукции.ВидПродукции.КодВидаПродукции КАК П000010000102,
	|	ВЫБОР
	...
	|ГДЕ
	|	(НЕ ХозрасчетныйОбороты.КорСчет В (&Счет41)) И (НЕ ХозрасчетныйОбороты.КорСчет ЕСТЬ NULL)
	|
	|СГРУППИРОВАТЬ ПО
	...
	ВывестиСведенияПодразделения(СведенияОПодразделении(Организация), "Раздел1");
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	СведенияОбАлкогольнойПродукции.ВидПродукции.Наименование КАК П000020000101,
	|	СведенияОбАлкогольнойПродукции.ВидПродукции.КодВидаПродукции КАК П000020000102,
	|	ВЫБОР
	|		КОГДА СведенияОбАлкогольнойПродукции.Номенклатура.Импортер = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|			ТОГДА ВЫРАЗИТЬ(СведенияОбАлкогольнойПродукции.Номенклатура.Производитель.НаименованиеПолное КАК СТРОКА(200))
	|		ИНАЧЕ ВЫРАЗИТЬ(СведенияОбАлкогольнойПродукции.Номенклатура.Импортер.НаименованиеПолное КАК СТРОКА(200))
	...
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ХозрасчетныйОбороты.СубконтоКт1 КАК Справочник.Контрагенты).ИНН, НЕОПРЕДЕЛЕНО) КАК П000020000107,
	...
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ХозрасчетныйОбороты.СубконтоКт1 КАК Справочник.Контрагенты).ИНН, НЕОПРЕДЕЛЕНО),
	...
	ВывестиСведенияПодразделения(СведенияОПодразделении(Организация), "Раздел2");
	ВывестиРазделВТабличныйДокумент("П0000200001", ТаблицаЗаполнения2);
	
	ТаблицаЗаполнения23 = ТаблицаЗаполнения2.Скопировать();
	
	Для Каждого Колонка Из ТаблицаЗаполнения23.Колонки Цикл
...
Процедура ЗаполнитьАвтоБухгалтерияКОРП(СписокОрганизаций)
	...
	|	АлкогольнаяПродукция.П000010000104 КАК П000010000104,
	|	АлкогольнаяПродукция.П000010000105 КАК П000010000105,
	|	ХозрасчетныйОстаткиНач.Подразделение КАК Подразделение,
	|	NULL,
	|	ХозрасчетныйОстаткиНач.Организация КАК Организация,
	|	NULL,
	|	СУММА(ХозрасчетныйОстаткиНач.КоличествоОстатокДт * АлкогольнаяПродукция.КоэффПересчетаДал) КАК П000010000106,
	|	0 КАК П000010000107,
	|	0 КАК П000010000108,
	...
	|	АлкогольнаяПродукция.П000010000104 КАК П000010000104,
	|	АлкогольнаяПродукция.П000010000105 КАК П000010000105,
	|	ХозрасчетныйОстаткиКон.Подразделение КАК Подразделение,
	|	NULL,
	|	ХозрасчетныйОстаткиКон.Организация КАК Организация,
	|	NULL,
	|	0 КАК П000010000106,
	|	0 КАК П000010000107,
	|	0 КАК П000010000108,
	...
	Для Каждого Подразделение Из ДеревоПоПодразделениям.Строки Цикл
		...
		Иначе
			РегламентированнаяОтчетность.роВставитьДополнительнуюСтраницу(ЭтаФорма, "Раздел1");
		КонецЕсли;
		ВывестиСведенияПодразделения(СведенияОПодразделении(Подразделение.СсылкаПодразделение), "Раздел1");
		ВывестиРазделВТабличныйДокумент("П0000100003", ТаблицаЗаполнения13);
		Расчет("Раздел1");
	КонецЦикла;
	...
	Для Каждого Подразделение Из ДеревоПоПодразделениям.Строки Цикл
		...
		Иначе
			РегламентированнаяОтчетность.роВставитьДополнительнуюСтраницу(ЭтаФорма, "Раздел2");
		КонецЕсли;
		ВывестиСведенияПодразделения(СведенияОПодразделении(Подразделение.СсылкаПодразделение), "Раздел2");
		ВывестиРазделВТабличныйДокумент("П0000200003", ТаблицаЗаполнения23);
		Расчет("Раздел2");
	КонецЦикла;
...
Процедура ЗаполнитьАвтоУПП(СписокОрганизаций)
	...
	|	СУММА(ВЫБОР
	|			КОГДА ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ВозвратТоваровПоставщику
	|				ИЛИ ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ВозвратТоваровПоставщикуИзНТТ
	|				ТОГДА ХозрасчетныйОбороты.КоличествоОборотКт * АлкогольнаяПродукция.КоэффПересчетаДал
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК П000010000116,
	|	СУММА(ВЫБОР
	|			КОГДА НЕ ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ОтчетОРозничныхПродажах
	|					И НЕ ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ВозвратТоваровПоставщику
	|					И НЕ ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ВозвратТоваровПоставщикуИзНТТ
	|					И НЕ ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ВозвратТоваровОтПокупателя
	|				ТОГДА ХозрасчетныйОбороты.КоличествоОборотКт * АлкогольнаяПродукция.КоэффПересчетаДал
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК П000010000115,
	...
	|			ТОГДА ЕСТЬNULL(ВЫРАЗИТЬ(ХозрасчетныйОбороты.Регистратор КАК Документ.ПоступлениеТоваровУслуг).ДатаВходящегоДокумента, ХозрасчетныйОбороты.Период)
	|		КОГДА ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ПоступлениеТоваровУслугВНТТ 
	|			ТОГДА ЕСТЬNULL(ВЫРАЗИТЬ(ХозрасчетныйОбороты.Регистратор КАК Документ.ПоступлениеТоваровУслугВНТТ).ДатаВходящегоДокумента, ХозрасчетныйОбороты.Период)
	|		ИНАЧЕ ХозрасчетныйОбороты.Период 
	|	КОНЕЦ КАК П000020000109,
	|	ВЫБОР 
	|		КОГДА ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ПоступлениеТоваровУслуг 
	|			ТОГДА ЕСТЬNULL(ВЫРАЗИТЬ(ХозрасчетныйОбороты.Регистратор КАК Документ.ПоступлениеТоваровУслуг).НомерВходящегоДокумента, ХозрасчетныйОбороты.Период)
	|		КОГДА ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ПоступлениеТоваровУслугВНТТ 
	|			ТОГДА ЕСТЬNULL(ВЫРАЗИТЬ(ХозрасчетныйОбороты.Регистратор КАК Документ.ПоступлениеТоваровУслугВНТТ).НомерВходящегоДокумента, ХозрасчетныйОбороты.Период)
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО 
	|	КОНЕЦ КАК П000020000110,
	|	ВЫБОР КОГДА (НЕ ТоварыОрганизаций.СерияНоменклатуры ЕСТЬ NULL) ТОГДА ТоварыОрганизаций.СерияНоменклатуры.НомерГТД КОНЕЦ КАК П000020000111,
	|	СУММА(ВЫБОР 
	|			КОГДА (НЕ ТоварыОрганизаций.СерияНоменклатуры ЕСТЬ NULL) 
	|				ТОГДА ТоварыОрганизаций.КоличествоОборот 
	|			ИНАЧЕ ХозрасчетныйОбороты.КоличествоОборотДт 
	|			КОНЕЦ * СведенияОбАлкогольнойПродукции.КоэффПересчетаДал) КАК П000020000112
	|ИЗ
	|	РегистрСведений.СведенияОбАлкогольнойПродукции КАК СведенияОбАлкогольнойПродукции
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(&ДатаНач, &ДатаКон, Регистратор, СчетДт В (&Счет41), &ВидСубконто, СчетКт В (&Счет60), &ВидКорСубконто, Организация В (&СписокОрганизаций)) КАК ХозрасчетныйОбороты
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыОрганизаций.Обороты(&ДатаНач, &ДатаКон, Регистратор, Организация В (&СписокОрганизаций)) КАК ТоварыОрганизаций
	|			ПО ХозрасчетныйОбороты.СубконтоДт1 = ТоварыОрганизаций.Номенклатура
	|				И ХозрасчетныйОбороты.Регистратор = ТоварыОрганизаций.Регистратор
	|		ПО СведенияОбАлкогольнойПродукции.Номенклатура = ХозрасчетныйОбороты.СубконтоДт1
	|ГДЕ
	|	СведенияОбАлкогольнойПродукции.ВидЛицензии = ЗНАЧЕНИЕ(Перечисление.ВидыЛицензийНаПродажуАлкоголя.Пиво)
	|
	...
	|	КОНЕЦ,
	|	ВЫРАЗИТЬ(ХозрасчетныйОбороты.СубконтоКт1.НаименованиеПолное КАК СТРОКА(200)),
	|	ВЫБОР КОГДА (НЕ ТоварыОрганизаций.СерияНоменклатуры ЕСТЬ NULL) ТОГДА ТоварыОрганизаций.СерияНоменклатуры.НомерГТД КОНЕЦ,
	|	ВЫБОР 
	|		КОГДА ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ПоступлениеТоваровУслуг 
	|			ТОГДА ЕСТЬNULL(ВЫРАЗИТЬ(ХозрасчетныйОбороты.Регистратор КАК Документ.ПоступлениеТоваровУслуг).НомерВходящегоДокумента, ХозрасчетныйОбороты.Период)
	|		КОГДА ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ПоступлениеТоваровУслугВНТТ 
...
Функция СведенияОПодразделении(Подразделение)
	...
	ИначеЕсли ТипЗнч(Подразделение) = Тип("СправочникСсылка.Организации") Тогда
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			ЭлементыАдреса.Регион          = РегламентированнаяОтчетность.КодРегионаПоНазванию(КонтактнаяИнформация.Поле2);
...
Процедура ВывестиСведенияПодразделения(СведенияОПодразделении, ИмяРаздела)
	...
	СведенияОбОП.КодРегиона = СведенияОПодразделении.ЭлементыАдреса.Регион;
...
Процедура РасставитьНомераСтраниц()
	...
	РасставитьНомераСтраницВМногостраничномРазделе("Раздел1", ПорядковыйНомер);
	РасставитьНомераСтраницВМногостраничномРазделе("Раздел2", ПорядковыйНомер);
...
Процедура УстановитьДоступностьПолейРаздел1()
	
	СтраницыРаздела = мСтруктураМногостраничныхРазделов.Раздел1;
	АктивнаяСтраница = СтраницыРаздела.Найти(Истина, "АктивнаяСтраница");
	...
	ИДКонфигурации = РегламентированнаяОтчетность.ИДКонфигурации();
	ПредусмотреноАвтозаполнение = (ИДКонфигурации <> "БГУ");
	
	ВариантРучногоВвода = ?(РазрешитьВвод, 0, 5);
	ВариантАвтозаполнения = ?(РазрешитьВвод, ?(ПредусмотреноАвтозаполнение, 3, 0), 5);
	ВариантАвторасчета = ?(РазрешитьВвод, ?(ФлажокОтклАвтоРасчет, 0, 4), 5);
	
	НомерСтроки = 1;
	Постфикс = Формат(НомерСтроки, "ЧГ=0");
	Пока ПТД_Раздел1.Области.Найти("П000010000101_" + Постфикс) <> Неопределено Цикл
		
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000101_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000102_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000103_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000104_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000105_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000106_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000107_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000108_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000109_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000110_" + Постфикс, ВариантАвторасчета);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000111_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000112_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000113_" + Постфикс, ВариантАвторасчета);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000114_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000115_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000116_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000117_" + Постфикс, ВариантАвторасчета);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000118_" + Постфикс, ВариантАвторасчета);
		
		НомерСтроки = НомерСтроки + 1;
		Постфикс = Формат(НомерСтроки, "ЧГ=0");
	КонецЦикла;
	
	ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000206", ВариантАвторасчета);
	ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000207", ВариантАвторасчета);
	ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000208", ВариантАвторасчета);
	ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000209", ВариантАвторасчета);
	ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000210", ВариантАвторасчета);
	ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000211", ВариантАвторасчета);
	ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000212", ВариантАвторасчета);
	ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000213", ВариантАвторасчета);
	ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000214", ВариантАвторасчета);
	ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000215", ВариантАвторасчета);
	ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000216", ВариантАвторасчета);
	ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000217", ВариантАвторасчета);
	ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000218", ВариантАвторасчета);
	
КонецПроцедуры

Процедура УстановитьДоступностьПолейРаздел2()
	
	СтраницыРаздела = мСтруктураМногостраничныхРазделов.Раздел2;
	АктивнаяСтраница = СтраницыРаздела.Найти(Истина, "АктивнаяСтраница");
	ИндексСтраницы = СтраницыРаздела.Индекс(АктивнаяСтраница);
	...
	НомерСтроки = 1;
	Постфикс = Формат(НомерСтроки, "ЧГ=0");
	Пока ПТД_Раздел2.Области.Найти("П000020000101_" + Постфикс) <> Неопределено Цикл
		
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел2, "П000020000101_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел2, "П000020000102_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел2, "П000020000103_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел2, "П000020000104_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел2, "П000020000105_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел2, "П000020000106_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел2, "П000020000107_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел2, "П000020000108_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел2, "П000020000109_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел2, "П000020000110_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел2, "П000020000111_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел2, "П000020000112_" + Постфикс, ВариантАвтозаполнения);
		
		НомерСтроки = НомерСтроки + 1;
		Постфикс = Формат(НомерСтроки, "ЧГ=0");
	КонецЦикла;
	
	ИзменитьВариантЗаполненияОбласти(ПТД_Раздел2, "П000020000212", ВариантАвторасчета);
...
Процедура УстановитьДоступностьКнопокДопСтрокТаблицы1(ПолеТабличногоДокумента)
	...
	Если ИндексСтраницы <> 0 Тогда
		...
		Если мСтруктураМногострочныхРазделов[мИдГруппы11].Колонки.Найти(ИмяКолонки) <> Неопределено Тогда
			ЭлементыФормы.КнопкаДобавитьДопСтрокиРаздел1.Доступность = Ложь;
			ЭлементыФормы.КнопкаУдалитьДопСтрокиРаздел1.Доступность =  Ложь;
		КонецЕсли;
...
Процедура УстановитьДоступностьКнопокДопСтрокТаблицы2(ПолеТабличногоДокумента)
	...
	Если ИндексСтраницы <> 0 Тогда
		...
		Если мСтруктураМногострочныхРазделов[мИдГруппы21].Колонки.Найти(ИмяКолонки) <> Неопределено Тогда
			ЭлементыФормы.КнопкаДобавитьДопСтрокиРаздел2.Доступность = Ложь;
			ЭлементыФормы.КнопкаУдалитьДопСтрокиРаздел2.Доступность =  Ложь;
		КонецЕсли;
...
Процедура ВвестиПоказатель(Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если Найти(КодПоказателя, "П000010000101") = 1 ИЛИ Найти(КодПоказателя, "П000010000301") = 1
	 ИЛИ Найти(КодПоказателя, "П000020000101") = 1 ИЛИ Найти(КодПоказателя, "П000020000301") = 1 Тогда
...
Процедура ЗаполнитьСвязанныеПоказатели(ИмяОбласти, РезультатВыбора)
	...
	ИначеЕсли Найти(ИмяОбласти, "П000020000101") = 1 Тогда
		...
		СвязаннаяОбласть = ПТД_Раздел2.Области[ИмяСвязаннойОбласти];
		СвязаннаяОбласть.Значение = РезультатВыбора.Код;
		Если ПустаяСтрока(РезультатВыбора.Код) Тогда
			ПТД_Раздел2.Области[ИмяОбласти].Значение = "";
		КонецЕсли;
...
Процедура Очистить() Экспорт
	...
	Если РегламентированнаяОтчетность.роОчистить(ЭтаФорма) Тогда
		...
		Для Каждого Страница Из мСтруктураМногостраничныхРазделов.Лицензии Цикл
			Страница.ДополнительныеДанные = Неопределено;
		КонецЦикла;
		Для Каждого Страница Из мСтруктураМногостраничныхРазделов.Раздел1 Цикл
			Страница.ДополнительныеДанные = Неопределено;
		КонецЦикла;
		Для Каждого Страница Из мСтруктураМногостраничныхРазделов.Раздел2 Цикл
			Страница.ДополнительныеДанные = Неопределено;
		КонецЦикла;
...
Функция ОпределитьИдМногострочногоРазделаПоАктивнойСтранице(ИмяЛиста = Неопределено) Экспорт
	...
	Если ИмяТекущейСтраницыПанели = "Раздел1" Тогда
		ИмяГруппы = РегламентированнаяОтчетность.роРазделТекущейОбласти(ЭтаФорма, ПТД_Раздел1.ТекущаяОбласть);
	ИначеЕсли ИмяТекущейСтраницыПанели = "Раздел2" Тогда
		ИмяГруппы = РегламентированнаяОтчетность.роРазделТекущейОбласти(ЭтаФорма, ПТД_Раздел2.ТекущаяОбласть);
	КонецЕсли;
...
Процедура ПоказатьПериод()
	...
	Заголовок = ЭтотОбъект.Метаданные().Формы.ОсновнаяФорма.Представление()
	            + ?(мДатаКонцаПериодаОтчета >= '20140101', ", сидра, пуаре и медовухи", "")
	            + " (за " + СтрПериодОтчета + ")";
...
Процедура ЗаполнитьДатуВЯчейках() Экспорт
	...
	Если РегламентированнаяОтчетность.ПустоеЗначение(ПТД_Титульный.Области.ОргИндивидуальныйПредприниматель.Значение) Тогда
		ДатаПодписиИП = "        ";
	Иначе
		Если ЭтаФорма.мДатаКонцаПериодаОтчета < '20140101' Тогда
	...
	Для НомЯчейки = 1 По 8 Цикл
		...
		ПТД_Титульный.Области["ДатаПодписиИП" + НомЯчейки].Значение = Сред(Формат(ДатаПодписиИП, "ДФ = 'ддММгггг'"), НомЯчейки, 1);
...
Процедура ЗаполнитьСведенияОбОрганизации(ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	Если НЕ ВПрограммеИзмененаДатаПодписи Тогда
		...
		ПредставлениеАдреса = РегламентированнаяОтчетность.ПредставлениеАдресаВФормате9Запятых(ЮридическийАдрес);
		Если ЭтоПБОЮЛ Тогда
			ПТД_Титульный.Области.ПоМестуОрганизации.Значение = "";
			ПТД_Титульный.Области.ПоМестуИндивидуальногоПредпринимателя.Значение = "V";
			ПТД_Титульный.Области.АдресИндивидуальногоПредпринимателя.Значение = ПредставлениеАдреса;
		Иначе
			ПТД_Титульный.Области.ПоМестуОрганизации.Значение = "V";
			ПТД_Титульный.Области.ПоМестуИндивидуальногоПредпринимателя.Значение = "";
			ПТД_Титульный.Области.АдресОрганизации.Значение = ПредставлениеАдреса;
		КонецЕсли;
	...
	Если ЭтоПБОЮЛ Тогда
		ФИОРук = "";
		ФИОИП = Организация.ИндивидуальныйПредприниматель.Наименование;
	Иначе
		ФИОРук = СведенияОбОрганизации.ФИОРук;
		ФИОИП = "";
	КонецЕсли;
	ФИОБух = СведенияОбОрганизации.ФИОБух;
	
	ЗаполнитьСведенияОбОтвественныхЛицах(ФИОРук, ФИОБух, ФИОИП);
	ЗаполнитьДатуВЯчейках();
	
КонецПроцедуры

Процедура ЗаполнитьСведенияОбОтвественныхЛицах(Знач ОргДиректор, Знач ОргБухгалтер, Знач ОргИндивидуальныйПредприниматель)
	
	ПТД_Титульный.Области.ОргДиректор.Значение = ОргДиректор;
	ПТД_Титульный.Области.ОргБухгалтер.Значение = ОргБухгалтер;
	ПТД_Титульный.Области.ОргИндивидуальныйПредприниматель.Значение = ОргИндивидуальныйПредприниматель;
	
	ПТД_Раздел1.Области.Руководитель.Значение = ОргДиректор;
	ПТД_Раздел1.Области.Бухгалтер.Значение = ОргБухгалтер;
	ПТД_Раздел1.Области.ИндивидуальныйПредприниматель.Значение = ОргИндивидуальныйПредприниматель;
	
	ПТД_Раздел2.Области.Руководитель.Значение = ОргДиректор;
	ПТД_Раздел2.Области.Бухгалтер.Значение = ОргБухгалтер;
	ПТД_Раздел2.Области.ИндивидуальныйПредприниматель.Значение = ОргИндивидуальныйПредприниматель;
...
Процедура ПронумероватьТаблицыДекларации()
	...
	// Раздел 1.
	ЭтоПерваяСтраница = мСтруктураМногостраничныхРазделов.Раздел1[0].АктивнаяСтраница;
	
	Если ЭтоПерваяСтраница Тогда
		ПроставитьНомераВТаблицеДекларация(ПТД_Раздел1, "П0000100001");
	КонецЕсли;
	
	ПроставитьНомераВТаблицеДекларация(ПТД_Раздел1, "П0000100003");
	
	// Раздел 2.
	ЭтоПерваяСтраница = мСтруктураМногостраничныхРазделов.Раздел2[0].АктивнаяСтраница;
	
	Если ЭтоПерваяСтраница Тогда
		ПроставитьНомераВТаблицеДекларация(ПТД_Раздел2, "П0000200001");
	КонецЕсли;
	
	ПроставитьНомераВТаблицеДекларация(ПТД_Раздел2, "П0000200003");
...
Процедура Печать(ВидПечати, ЧтоИскать = Неопределено) Экспорт
	...
	Для Каждого СтрокаУровня1 Из мДеревоВыбранныхСтраниц.Строки Цикл
		...
		ИначеЕсли ЧтоИскать = Неопределено И ИмяСтраницыПанели = "Раздел1" Тогда
			СтруктураПараметров.Вставить("ИмяСтраницыПанели", ИмяСтраницыПанели);
			ВывестиНаПечатьРаздел1(СтруктураПараметров);
		ИначеЕсли ЧтоИскать = Неопределено И ИмяСтраницыПанели = "Раздел2" Тогда
			СтруктураПараметров.Вставить("ИмяСтраницыПанели", ИмяСтраницыПанели);
...
Процедура ВывестиНаПечатьРаздел1(СтруктураПараметровСтраницы)
	...
	Если мДатаКонцаПериодаОтчета < '20140101' Тогда
		МакетПечати = ПолучитьМакет("ФормаПечати2012Кв3_Раздел1");
	Иначе
		МакетПечати = ПолучитьМакет("ФормаПечати2012Кв3_Раздел1_20140101");
	КонецЕсли;
	...
	Если ЭтоПБОЮЛ Тогда
		...
		Адрес =  СокрЛП(ПТД_Титульный.Области.АдресИндивидуальногоПредпринимателя.Значение);
	...
	Иначе
		...
		Адрес = СокрЛП(ПТД_Титульный.Области.АдресОрганизации.Значение);
	...
	ДанныеПоОрганизации = мСтруктураМногостраничныхРазделов.Раздел1[0].ДанныеДопСтрок.П0000100001.Скопировать();
	...
	Для Каждого Лист Из мСтруктураМногостраничныхРазделов.Раздел1 Цикл
	...
	Область_Подвал.Области.Руководитель.Значение = мСтруктураМногостраничныхРазделов.Раздел1[0].Данные.Руководитель;
	Область_Подвал.Области.Бухгалтер.Значение = мСтруктураМногостраничныхРазделов.Раздел1[0].Данные.Бухгалтер;
	Область_Подвал.Области.ИндивидуальныйПредприниматель.Значение = мСтруктураМногостраничныхРазделов.Раздел1[0].Данные.ИндивидуальныйПредприниматель;
...
Процедура ВывестиНаПечатьРаздел2(СтруктураПараметровСтраницы)
	...
	МакетПечати = ПолучитьМакет("ФормаПечати2012Кв3_Раздел2");
	...
	ДанныеПоОрганизации = мСтруктураМногостраничныхРазделов.Раздел2[0].ДанныеДопСтрок.П0000200001.Скопировать();
	Для Каждого СтрокаДанных Из ДанныеПоОрганизации Цикл
		Область_Строка = МакетПечати.ПолучитьОбласть("Строка");
		Для Каждого Колонка Из ДанныеПоОрганизации.Колонки Цикл
			ИмяКолонки = Колонка.Имя;
	...
	Для Каждого Лист Из мСтруктураМногостраничныхРазделов.Раздел2 Цикл
		
		Область_ПустаяСтрока = МакетПечати.ПолучитьОбласть("ПустаяСтрока");
		ПечатнаяФорма.Вывести(Область_ПустаяСтрока);
		
		ДанныеЛиста = Лист.Данные;
		ДанныеПоПодразделению = Лист.ДанныеДопСтрок.П0000200003.Скопировать();
		Область_РеквизитыОрганизации = МакетПечати.ПолучитьОбласть("РеквизитыОрганизации");
		Если ЭтоПБОЮЛ Тогда
			Область_РеквизитыОрганизации.Области.ЗаголовокРеквизитов.Текст = "Сведения по по объекту торговли индивидуального предпринимателя (с указанием адреса)";
		Иначе
	...
	Область_Подвал = МакетПечати.ПолучитьОбласть("Подвал");
	Область_Подвал.Области.Руководитель.Значение = мСтруктураМногостраничныхРазделов.Раздел2[0].Данные.Руководитель;
	Область_Подвал.Области.Бухгалтер.Значение = мСтруктураМногостраничныхРазделов.Раздел2[0].Данные.Бухгалтер;
	Область_Подвал.Области.ИндивидуальныйПредприниматель.Значение = мСтруктураМногостраничныхРазделов.Раздел2[0].Данные.ИндивидуальныйПредприниматель;
	ПечатнаяФорма.Вывести(Область_Подвал);
...
Процедура РасчетРаздел1(Знач МестоРазмещенияДанных = Неопределено)
	...
	Если МестоРазмещенияДанных = Неопределено Тогда
		МестоРазмещенияДанных = ПТД_Раздел1;
	КонецЕсли;
	
	РасчетРаздел1Таблица(МестоРазмещенияДанных, мИдГруппы11, "П0000100001", "П0000100002");
	РасчетРаздел1Таблица(МестоРазмещенияДанных, мИдГруппы12, "П0000100003", "П0000100004");
	
КонецПроцедуры

Процедура РасчетРаздел1Таблица(МестоРазмещенияДанных, ИдГруппы, КодСекции, КодИтога)
	...
	Для Инд = 1 По КоличествоСтрок Цикл
		...
		Графа07 = ПолучитьДанныеИзМестаРазмещения(МестоРазмещенияДанных, КодСекции + "07" + ПостфиксСтроки);
		Графа08 = ПолучитьДанныеИзМестаРазмещения(МестоРазмещенияДанных, КодСекции + "08" + ПостфиксСтроки);
		Графа09 = ПолучитьДанныеИзМестаРазмещения(МестоРазмещенияДанных, КодСекции + "09" + ПостфиксСтроки);
		
		ЗначениеГрафы10 = Графа07 + Графа08 + Графа09 ;
		ПоместитьДанныеПоМестуРазмещения(МестоРазмещенияДанных, КодСекции + "10" + ПостфиксСтроки, ЗначениеГрафы10);
		
		Графа10 = ПолучитьДанныеИзМестаРазмещения(МестоРазмещенияДанных, КодСекции + "10" + ПостфиксСтроки);
		Графа11 = ПолучитьДанныеИзМестаРазмещения(МестоРазмещенияДанных, КодСекции + "11" + ПостфиксСтроки);
		Графа12 = ПолучитьДанныеИзМестаРазмещения(МестоРазмещенияДанных, КодСекции + "12" + ПостфиксСтроки);
		
		ЗначениеГрафы13 = Графа10 + Графа11 + Графа12;
		ПоместитьДанныеПоМестуРазмещения(МестоРазмещенияДанных, КодСекции + "13" + ПостфиксСтроки, ЗначениеГрафы13);
		
		Графа14 = ПолучитьДанныеИзМестаРазмещения(МестоРазмещенияДанных, КодСекции + "14" + ПостфиксСтроки);
		Графа15 = ПолучитьДанныеИзМестаРазмещения(МестоРазмещенияДанных, КодСекции + "15" + ПостфиксСтроки);
		Графа16 = ПолучитьДанныеИзМестаРазмещения(МестоРазмещенияДанных, КодСекции + "16" + ПостфиксСтроки);
		
		ЗначениеГрафы17 =  Графа14 + Графа15 + Графа16;
		ПоместитьДанныеПоМестуРазмещения(МестоРазмещенияДанных, КодСекции + "17" + ПостфиксСтроки, ЗначениеГрафы17);
		
		Графа06 = ПолучитьДанныеИзМестаРазмещения(МестоРазмещенияДанных, КодСекции + "06" + ПостфиксСтроки);
		Графа13 = ПолучитьДанныеИзМестаРазмещения(МестоРазмещенияДанных, КодСекции + "13" + ПостфиксСтроки);
		Графа17 = ПолучитьДанныеИзМестаРазмещения(МестоРазмещенияДанных, КодСекции + "17" + ПостфиксСтроки);
		
		ЗначениеГрафы18 = Графа06 + Графа13 - Графа17;
		ПоместитьДанныеПоМестуРазмещения(МестоРазмещенияДанных, КодСекции + "18" + ПостфиксСтроки, ЗначениеГрафы18);
...
Процедура РасчетРаздел2(Знач МестоРазмещенияДанных = Неопределено)
	...
	Если МестоРазмещенияДанных = Неопределено Тогда
		МестоРазмещенияДанных = ПТД_Раздел2;
	КонецЕсли;
	
	РасчетРаздел2Таблица(МестоРазмещенияДанных, мИдГруппы21, "П0000200001", "П0000200002");
	РасчетРаздел2Таблица(МестоРазмещенияДанных, мИдГруппы22, "П0000200003", "П0000200004");
	
КонецПроцедуры

Процедура РасчетРаздел2Таблица(МестоРазмещенияДанных, ИдГруппы, КодСекции, КодИтога)
	
	СписокСуммируемыхГраф = "12";
...
Процедура ИнициализироватьПоляТабличныхДокументов()
	...
	Если мДатаКонцаПериодаОтчета < '20140101' Тогда
		Макет = ЭтотОбъект.ПолучитьМакет("ФормаОтчета2012Кв3_Раздел1");
	Иначе
		Макет = ЭтотОбъект.ПолучитьМакет("ФормаОтчета2012Кв3_Раздел1_20140101");
	КонецЕсли;
	ПТД_Раздел1 = ЭлементыФормы.ПолеТабличногоДокументаРаздел1;
	ПТД_Раздел1.Очистить();
	ПТД_Раздел1.Вывести(Макет);
	
	Макет = ЭтотОбъект.ПолучитьМакет("ФормаОтчета2012Кв3_Раздел2");
...
Процедура ИнициализироватьПоляВводаТабличныхДокументов()
	...
	ПТД_Раздел1.Области.П000010000102_1.ЭлементУправления.Маска = "999";
	ПТД_Раздел1.Области.П000010000302_1.ЭлементУправления.Маска = "999";
	
	ПТД_Раздел2.Области.П000020000102_1.ЭлементУправления.Маска = "999";
	ПТД_Раздел2.Области.П000020000302_1.ЭлементУправления.Маска = "999";
	
	МаскаИНН10 = "9999999999";
	МаскаИНН12 = "999999999999";
	МаскаКПП = "999999999";
	
	ПТД_Раздел1.Области.П000010000105_1.ЭлементУправления.Маска = МаскаКПП;
	ПТД_Раздел1.Области.П000010000305_1.ЭлементУправления.Маска = МаскаКПП;
	
	ПТД_Раздел2.Области.П000020000105_1.ЭлементУправления.Маска = МаскаКПП;
	ПТД_Раздел2.Области.П000020000305_1.ЭлементУправления.Маска = МаскаКПП;
	
	ПТД_Раздел2.Области.П000020000107_1.ЭлементУправления.Маска = МаскаИНН12;
	ПТД_Раздел2.Области.П000020000307_1.ЭлементУправления.Маска = МаскаИНН12;
	
	ПТД_Раздел2.Области.П000020000108_1.ЭлементУправления.Маска = МаскаКПП;
	ПТД_Раздел2.Области.П000020000308_1.ЭлементУправления.Маска = МаскаКПП;
...
Процедура ИнициализироватьДополнительныеДанные()
	...
	Если мСтруктураМногостраничныхРазделов.Раздел1.Колонки.Найти("ДополнительныеДанные") = Неопределено Тогда
		мСтруктураМногостраничныхРазделов.Раздел1.Колонки.Добавить("ДополнительныеДанные");
	КонецЕсли;
	
	Если мСтруктураМногостраничныхРазделов.Раздел2.Колонки.Найти("ДополнительныеДанные") = Неопределено Тогда
		мСтруктураМногостраничныхРазделов.Раздел2.Колонки.Добавить("ДополнительныеДанные");
	КонецЕсли;
...
Функция ПроверитьПоказатели() Экспорт
	...
	// Сформируем списки выбора на случай запуска выгрузки или проверки выгрузки
	// из 1С-Отчетности напрямую, не из формы Отчета.
	СформироватьСпискиВыбора();
...
Функция ОтчетЗаполненКорректно(Параметры)
	...
	ПроверитьЗаполнениеРаздел1(Параметры, СтатусПроверки);
	
	Если ТаблицаСообщений.Количество() <=  100 Тогда
		ПроверитьЗаполнениеРаздел2(Параметры, СтатусПроверки);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеТитульный(Параметры, СтатусПроверки)
	...
	Если Параметры.Свойство("ИННЮЛ") Тогда
	...
	Иначе
		Если ПустаяСтрока(Параметры.ИННФЛ) Тогда
			...
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Титульный", "ИНН1_1");
		ИначеЕсли СтрДлина(СокрЛП(Параметры.ИННФЛ))<> 12 Тогда
	...
	Если Параметры.ЭтоПБОЮЛ Тогда
		УказаныФИО = НЕ ПустаяСтрока(Параметры.ИПФамилия) И НЕ ПустаяСтрока(Параметры.ИПИмя);
		Если НЕ УказаныФИО Тогда
			ТекстОписанияОшибки = "Неверно указаны фамилия, имя и отчество индивидуального предпринимателя!";
...
Процедура ПроверитьЗаполнениеРаздел1(ПараметрыВыгрузки, СтатусПроверки)
	...
	Если мСохраненныйДок.ДатаОкончания > '20140901' Тогда
		...
		Для Каждого ЛистРаздела Из ДанныеРаздела Цикл
			...
			СведенияОбОП = Неопределено;
			Если ТипЗнч(ЛистРаздела.ДополнительныеДанные) = Тип("Структура") 
				И ЛистРаздела.ДополнительныеДанные.Свойство("СведенияОбОП", СведенияОбОП) Тогда
				
				Если ЗначениеЗаполнено(СведенияОбОП.КПП) и ЗначениеЗаполнено(СведенияОбОП.Наименование) Тогда
		...
		Для Каждого СтрокаТЗ Из КопияДанныеОПТЗ_Р1 Цикл
			...
			Если СтрокаТЗ.Колво > 1 Тогда
				...
				Если НайденныеОП.Количество() > 0 Тогда
					...
					ТекстОписанияОшибки = "В Разделе 1 декларации страницы №№ " + ЛистыПовторы + "  заполнены по одному и тому же предприятию: " + ЭлНайденныеОП.НаимОП + ", (КПП " + ЭлНайденныеОП.КППОП + ")";
	...
	Для Каждого ЛистРаздела Из ДанныеРаздела Цикл
		...
		Если ПустаяСтрока(ДанныеЛиста.ОбособленноеПодразделение) Тогда
			ТекстОписанияОшибки = "В Разделе 1 декларации, на листе " + НомерЛистаСтроковый + ", не указаны реквизиты обособленного подразделения!";
		...
		Иначе
			...
			Если ТипЗнч(ЛистРаздела.ДополнительныеДанные) = Тип("Структура") И ЛистРаздела.ДополнительныеДанные.Свойство("СведенияОбОП", СведенияОбОП) Тогда
				...
				Если ПустаяСтрока(ИНН) Тогда
					ТекстОписанияОшибки = "В Разделе 1 декларации, на листе " + НомерЛистаСтроковый + ", не указан ИНН обособленного подразделения!";
					СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", "ОбособленноеПодразделение", НомерЛиста);
				ИначеЕсли НЕ ЭтоПБОЮЛ И СтрДлина(ИНН) <> 10 Тогда
					ТекстОписанияОшибки = "В Разделе 1 декларации, на листе " + НомерЛистаСтроковый + ", неверно указан ИНН обособленного подразделения!";
					СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", "ОбособленноеПодразделение", НомерЛиста);
				ИначеЕсли ЭтоПБОЮЛ И СтрДлина(ИНН) <> 12 Тогда
					ТекстОписанияОшибки = "В Разделе 1 декларации, на листе " + НомерЛистаСтроковый + ", неверно указан ИНН индивидуального предпринимателя!";
					СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", "ОбособленноеПодразделение", НомерЛиста);
				КонецЕсли;
				
				Если НЕ ЭтоПБОЮЛ Тогда
					Если ПустаяСтрока(КПП) Тогда
						ТекстОписанияОшибки = "В Разделе 1 декларации, на листе " + НомерЛистаСтроковый + ", не указан КПП обособленного подразделения!";
						СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", "ОбособленноеПодразделение", НомерЛиста);
					ИначеЕсли СтрДлина(КПП) <> 9 Тогда
						ТекстОписанияОшибки = "В Разделе 1 декларации, на листе " + НомерЛистаСтроковый + ", неверно указан КПП обособленного подразделения!";
						СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", "ОбособленноеПодразделение", НомерЛиста);
					КонецЕсли;
				КонецЕсли;
								
				Если ПустаяСтрока(СведенияОбОП.Наименование) Тогда
					ТекстОписанияОшибки = "В Разделе 1 декларации, на листе " + НомерЛистаСтроковый + ", не указано наименование обособленного подразделения!";
					СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", "ОбособленноеПодразделение", НомерЛиста);
				КонецЕсли;
				
				Если ПустаяСтрока(СведенияОбОП.ПредставлениеАдреса) Тогда
					ТекстОписанияОшибки = "В Разделе 1 декларации, на листе " + НомерЛистаСтроковый + ", не указан адрес обособленного подразделения!";
					СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", "ОбособленноеПодразделение", НомерЛиста);
				ИначеЕсли ПустаяСтрока(СведенияОбОП.КодРегиона) Тогда
					ТекстОписанияОшибки = "В Разделе 1 декларации, на листе " + НомерЛистаСтроковый + ", в адресе обособленного подразделения не указан регион!";
					СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", "ОбособленноеПодразделение", НомерЛиста);
				КонецЕсли;
			Иначе
				ТекстОписанияОшибки = "В Разделе 1 декларации, на листе " + НомерЛистаСтроковый + ", нарушена внутренняя структура реквизитов обособленного подразделения, требуется повторный ввод данных!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", "ОбособленноеПодразделение", НомерЛиста);
			КонецЕсли;
		КонецЕсли;
		...
		Если ТипЗнч(ЛистРаздела.ДополнительныеДанные) = Тип("Структура") Тогда
			ЛистРаздела.ДополнительныеДанные.Свойство(мИдГруппы12, ДопДанные);
		КонецЕсли;
		ПроверитьЗаполнениеТаблицыРаздел1(СтатусПроверки, мИдГруппы12, ДанныеДопСтрок, ДопДанные, НомерЛиста, СведенияОбОП, ЕстьЗаполненныйЛистРаздела);
...
Процедура ПроверитьЗаполнениеТаблицыРаздел1(СтатусПроверки, ИдГруппы, ДанныеДопСтрок, ДополнительныеДанные, НомерЛиста, СведенияОбОП = Неопределено, ЕстьЗаполненныйЛистРаздела = Ложь)
	...
	Для Каждого СтрокаДекларации Из ДанныеДопСтрок Цикл
		...
		// Производитель-импортер - может быть как российским так и иностранным контрагентом.
		...
		// Армения    - УНН - 8 цифр.		
		Если ПустаяСтрока(ИНН) Тогда
			ТекстОписанияОшибки = "В Разделе 1 декларации, на листе " + НомерЛистаСтроковый + ", в строке " + НомерСтрокиСтроковый + ", в таблице "+ ИмяТаблицы + ", не указан ИНН производителя/импортера!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", ИмяПоказателяИНН, НомерЛиста);
		ИначеЕсли (ДлинаИНН <> 10) и (ДлинаИНН <> 12) и (ДлинаИНН <> 9) и (ДлинаИНН <> 8)  и (ДлинаИНН <> 14) Тогда
			ТекстОписанияОшибки = "В Разделе 1 декларации, на листе " + НомерЛистаСтроковый + ", в строке " + НомерСтрокиСтроковый + ", в таблице "+ ИмяТаблицы + ", неверно указан ИНН производителя/импортера!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", ИмяПоказателяИНН, НомерЛиста);
		КонецЕсли;
		...
		Если ДлинаИНН = 10 Тогда
			// это Российский ИНН.
			Если ПустаяСтрока(КПП) Тогда
				ТекстОписанияОшибки = "В Разделе 1 декларации, на листе " + НомерЛистаСтроковый + ", в строке " + НомерСтрокиСтроковый + ", в таблице "+ ИмяТаблицы + ", не указан КПП производителя/импортера!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", ИмяПоказателяКПП, НомерЛиста);
			ИначеЕсли СтрДлина(КПП) <> 9 Тогда
				ТекстОписанияОшибки = "В Разделе 1 декларации, на листе " + НомерЛистаСтроковый + ", в строке " + НомерСтрокиСтроковый + ", в таблице "+ ИмяТаблицы + ", неверно указан КПП производителя/импортера!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", ИмяПоказателяКПП, НомерЛиста);
			КонецЕсли;
		Иначе
			// Либо иностранный ИНН, либо ИНН физлица. КПП должен быть пуст.
			Если НЕ ПустаяСтрока(КПП) Тогда
				ТекстОписанияОшибки = "В Разделе 1 декларации, на листе " + НомерЛистаСтроковый + ", в строке " + НомерСтрокиСтроковый + ", в таблице "+ ИмяТаблицы + ", КПП производителя/импортера должен быть пуст!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", ИмяПоказателяКПП, НомерЛиста);
			КонецЕсли;
		КонецЕсли;
		
		// Заполняем Таблицу Производителей раздела 1.
		Если (ИмяТаблицы = """по обособленному подразделению""") и НЕ(СведенияОбОП = Неопределено) Тогда
			Если ЗначениеЗаполнено(НаименованиеПроизводителя) и ЗначениеЗаполнено(ИНН)
				и ЗначениеЗаполнено(СведенияОбОП.ИНН)
				и ЗначениеЗаполнено(СведенияОбОП.КПП) и ЗначениеЗаполнено(СведенияОбОП.Наименование) Тогда
	...
	Если мСохраненныйДок.ДатаОкончания > '20140901' Тогда
		...
		Для Каждого СтрокаТЗ Из КопияДанныеОПТЗ_Р2 Цикл
			...
			Если СтрокаТЗ.Колво > 1 Тогда
				...
				Если НайденныеОП.Количество() > 0 Тогда
					...
					ТекстОписанияОшибки = "В Разделе 2 декларации страницы №№ " + ЛистыПовторы + " заполнены по одному и тому же предприятию: " + ЭлНайденныеОП.НаимОП + ", (КПП " + ЭлНайденныеОП.КППОП + ")";
		...
		Для Каждого СтрокаТЗ Из КопияДанныеОПТЗ_Р2 Цикл
			...
			ТекстОписанияОшибки = "В Разделе 2 декларации, на странице № " + ЛистРаздела2 + ", заполнены данные по обособленному подразделению " + СтрокаТЗ.НаимОП 
								  + ", (КПП " + СтрокаТЗ.КППОП + "), но это обособленное подразделение не найдено в Разделе 1!";
	...
	Для Каждого ЛистРаздела Из ДанныеРаздела Цикл
		...
		Иначе
			...
			Если ТипЗнч(ЛистРаздела.ДополнительныеДанные) = Тип("Структура") И ЛистРаздела.ДополнительныеДанные.Свойство("СведенияОбОП", СведенияОбОП) Тогда
				...
				Если ПустаяСтрока(ИНН) Тогда
					ТекстОписанияОшибки = "В Разделе 2 декларации, на листе " + НомерЛистаСтроковый + ", не указан ИНН обособленного подразделения!";
					СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", "ОбособленноеПодразделение", НомерЛиста);
				ИначеЕсли НЕ ЭтоПБОЮЛ И СтрДлина(ИНН) <> 10 Тогда
					ТекстОписанияОшибки = "В Разделе 2 декларации, на листе " + НомерЛистаСтроковый + ", неверно указан ИНН обособленного подразделения!";
					СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", "ОбособленноеПодразделение", НомерЛиста);
				ИначеЕсли ЭтоПБОЮЛ И СтрДлина(ИНН) <> 12 Тогда
					ТекстОписанияОшибки = "В Разделе 2 декларации, на листе " + НомерЛистаСтроковый + ", неверно указан ИНН индивидуального предпринимателя!";
					СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", "ОбособленноеПодразделение", НомерЛиста);
				КонецЕсли;
				
				Если НЕ ЭтоПБОЮЛ Тогда
					Если ПустаяСтрока(КПП) Тогда
						ТекстОписанияОшибки = "В Разделе 2 декларации, на листе " + НомерЛистаСтроковый + ", не указан КПП обособленного подразделения!";
						СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", "ОбособленноеПодразделение", НомерЛиста);
					ИначеЕсли СтрДлина(КПП) <> 9 Тогда
						ТекстОписанияОшибки = "В Разделе 2 декларации, на листе " + НомерЛистаСтроковый + ", неверно указан КПП обособленного подразделения!";
						СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", "ОбособленноеПодразделение", НомерЛиста);
					КонецЕсли;
				КонецЕсли;
								
				Если ПустаяСтрока(СведенияОбОП.Наименование) Тогда
					ТекстОписанияОшибки = "В Разделе 2 декларации, на листе " + НомерЛистаСтроковый + ", не указано наименование обособленного подразделения!";
					СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", "ОбособленноеПодразделение", НомерЛиста);
				КонецЕсли;
				
				Если ПустаяСтрока(СведенияОбОП.ПредставлениеАдреса) Тогда
					ТекстОписанияОшибки = "В Разделе 2 декларации, на листе " + НомерЛистаСтроковый + ", не указан адрес обособленного подразделения!";
					СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", "ОбособленноеПодразделение", НомерЛиста);
				ИначеЕсли ПустаяСтрока(СведенияОбОП.КодРегиона) Тогда
					ТекстОписанияОшибки = "В Разделе 2 декларации, на листе " + НомерЛистаСтроковый + ", в адресе обособленного подразделения не указан регион!";
					СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", "ОбособленноеПодразделение", НомерЛиста);
				КонецЕсли;
			Иначе
				ТекстОписанияОшибки = "В Разделе 2 декларации, на листе " + НомерЛистаСтроковый + ", нарушена внутренняя структура реквизитов обособленного подразделения, требуется повторный ввод данных!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", "ОбособленноеПодразделение", НомерЛиста);
			КонецЕсли;
		...
		Если ТипЗнч(ЛистРаздела.ДополнительныеДанные) = Тип("Структура") Тогда
			ЛистРаздела.ДополнительныеДанные.Свойство(мИдГруппы22, ДопДанные);
		КонецЕсли;
		ПроверитьЗаполнениеТаблицыРаздел2(СтатусПроверки, мИдГруппы22, ДанныеДопСтрок, ДопДанные, НомерЛиста, СведенияОбОП, ЕстьЗаполненныеСтроки);
...
Процедура ПроверитьЗаполнениеТаблицыРаздел2(СтатусПроверки, ИдГруппы, ДанныеДопСтрок, ДополнительныеДанные, НомерЛиста, СведенияОбОП = Неопределено, ЕстьЗаполненныйЛистРаздела = Ложь)
	...
	Для Каждого СтрокаДекларации Из ДанныеДопСтрок Цикл
		...
		// Армения    - УНН - 8 цифр.
		Если ПустаяСтрока(ИНН) Тогда
			ТекстОписанияОшибки = "В Разделе 2 декларации, на листе " + НомерЛистаСтроковый + ", в строке " + НомерСтрокиСтроковый + ", в таблице "+ ИмяТаблицы + ", не указан ИНН производителя/импортера!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателяИНН, НомерЛиста);
		ИначеЕсли (ДлинаИНН <> 10) и (ДлинаИНН <> 12) и (ДлинаИНН <> 9) и (ДлинаИНН <> 8)  и (ДлинаИНН <> 14) Тогда
			ТекстОписанияОшибки = "В Разделе 2 декларации, на листе " + НомерЛистаСтроковый + ", в строке " + НомерСтрокиСтроковый + ", в таблице "+ ИмяТаблицы + ", неверно указан ИНН производителя/импортера!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателяИНН, НомерЛиста);
		КонецЕсли;
		...
		Если ДлинаИНН = 10 Тогда
			// это Российский ИНН.
			Если ПустаяСтрока(КПП) Тогда
				ТекстОписанияОшибки = "В Разделе 2 декларации, на листе " + НомерЛистаСтроковый + ", в строке " + НомерСтрокиСтроковый + ", в таблице "+ ИмяТаблицы + ", не указан КПП производителя/импортера!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателяКПП, НомерЛиста);
			ИначеЕсли СтрДлина(КПП) <> 9 Тогда
				ТекстОписанияОшибки = "В Разделе 2 декларации, на листе " + НомерЛистаСтроковый + ", в строке " + НомерСтрокиСтроковый + ", в таблице "+ ИмяТаблицы + ", неверно указан КПП производителя/импортера!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателяКПП, НомерЛиста);
			КонецЕсли;
		Иначе
			// Либо иностранный ИНН, либо ИНН физлица. КПП должен быть пуст.
			Если НЕ ПустаяСтрока(КПП) Тогда
				ТекстОписанияОшибки = "В Разделе 2 декларации, на листе " + НомерЛистаСтроковый + ", в строке " + НомерСтрокиСтроковый + ", в таблице "+ ИмяТаблицы + ", КПП производителя/импортера должен быть пуст!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателяКПП, НомерЛиста);
			КонецЕсли;
		КонецЕсли;
			...
			Если ЗначениеЗаполнено(НаименованиеПроизводителя) и ЗначениеЗаполнено(ИННПроизводителя)
				...
				Если НайденныеПроизводители.Количество() = 0 Тогда
					ТекстОписанияОшибки = "В Разделе 2 декларации, на листе " + НомерЛистаСтроковый + ", в строке " + НомерСтрокиСтроковый + ", в таблице "+ ИмяТаблицы + ", " 
										+ "указан Производитель/Импортер " + НаименованиеПроизводителя + " (ИНН "+ ИННПроизводителя
										+ ", КПП " + КПППроизводителя + "), которого нет по тому же обособленному подразделению в Разделе 1";
		...
		// Армения    - УНН - 8 цифр.
		Если ПустаяСтрока(ИНН) Тогда
			ТекстОписанияОшибки = "В Разделе 2 декларации, на листе " + НомерЛистаСтроковый + ", в строке " + НомерСтрокиСтроковый + ", в таблице "+ ИмяТаблицы + ", не указан ИНН поставщика!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателяИНН, НомерЛиста);
		ИначеЕсли (ДлинаИНН <> 10) и (ДлинаИНН <> 12) и (ДлинаИНН <> 9) и (ДлинаИНН <> 8)  и (ДлинаИНН <> 14) Тогда
			ТекстОписанияОшибки = "В Разделе 2 декларации, на листе " + НомерЛистаСтроковый + ", в строке " + НомерСтрокиСтроковый + ", в таблице "+ ИмяТаблицы + ", неверно указан ИНН поставщика!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателяИНН, НомерЛиста);
		КонецЕсли;
		...
		Если ДлинаИНН = 10 Тогда
			// это Российский ИНН.
			Если ПустаяСтрока(КПП) Тогда
				ТекстОписанияОшибки = "В Разделе 2 декларации, на листе " + НомерЛистаСтроковый + ", в строке " + НомерСтрокиСтроковый + ", в таблице "+ ИмяТаблицы + ", не указан КПП поставщика!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателяКПП, НомерЛиста);
			ИначеЕсли СтрДлина(КПП) <> 9 Тогда
				ТекстОписанияОшибки = "В Разделе 2 декларации, на листе " + НомерЛистаСтроковый + ", в строке " + НомерСтрокиСтроковый + ", в таблице "+ ИмяТаблицы + ", неверно указан КПП поставщика!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателяКПП, НомерЛиста);
			КонецЕсли;
		Иначе
			// Либо иностранный ИНН, либо ИНН физлица. КПП должен быть пуст.
			Если НЕ ПустаяСтрока(КПП) Тогда
				ТекстОписанияОшибки = "В Разделе 2 декларации, на листе " + НомерЛистаСтроковый + ", в строке " + НомерСтрокиСтроковый + ", в таблице "+ ИмяТаблицы + ", КПП поставщика должен быть пуст!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателяКПП, НомерЛиста);
			КонецЕсли;
		КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Имясобытия = "Изменение статуса отправки регламентированного отчета" И ЗначениеЗаполнено(Параметр) И мСохраненныйДок <> Неопределено И Источник = мСохраненныйДок.Ссылка Тогда
		...
		Если мСохраненныйДок <> Неопределено И КонтекстЭДО <> Неопределено Тогда
...
Функция СформироватьСтруктуруПоказателейИСтруктуруПараметров()
	...
	Если ЭтоПБОЮЛ Тогда
		...
		ФИОИП = РазложитьФИО(ПоказателиТитульный.ОргИндивидуальныйПредприниматель);
		
		Параметры.Вставить("РуководительФамилия",  ФИОИП.Фамилия);
		Параметры.Вставить("РуководительИмя",      ФИОИП.Имя);
		Параметры.Вставить("РуководительОтчество", ФИОИП.Отчество);
		
		Параметры.Вставить("ИПФамилия",  ФИОИП.Фамилия);
		Параметры.Вставить("ИПИмя",      ФИОИП.Имя);
		Параметры.Вставить("ИПОтчество", ФИОИП.Отчество);
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель, КорневойУзел, Параметры)
	...
	Для Каждого Страница Из мДанныеОтчета.Раздел1 Цикл
		...
		Если мДанныеОтчета.Раздел1.Колонки.Найти("ДополнительныеДанные") = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		ТаблицаДвиженийОП = Страница.ДанныеДопСтрок.П0000100003.Скопировать();
		...
		ТаблицаПоставокОП = мДанныеОтчета.Раздел2[0].ДанныеДопСтрок.П0000200003.СкопироватьКолонки();
		Для Каждого СтраницаТабл2 Из мДанныеОтчета.Раздел2 Цикл
			СведенияОбОПРаздела2 = ДополнительныеДанныеПоАдресу(СтраницаТабл2.ДополнительныеДанные, "СведенияОбОП");
			ИдентификаторОПРаздела2 = ИдентификаторОП(СведенияОбОПРаздела2);
			Если ИдентификаторОПРаздела2 = ИдентификаторОП Тогда
				ТаблицаПоставокОП = СтраницаТабл2.ДанныеДопСтрок.П0000200003.Скопировать();
				Прервать;
			КонецЕсли;
		КонецЦикла;
	...
	Для Каждого СтрокаКодаВидаПродукции Из КодыВидовПродукции Цикл
		...
		Для Каждого СтрокаСведенийОПроизводителеИмпортере Из СведенияОПроизводителеИмпортере Цикл
			...
			Для Каждого СтрокаСведенийОПоставщике Из СведенияОПоставщиках Цикл
				...
				УдалитьУзел(ОбразецУзлаПродукция);
				
			КонецЦикла;
			
			УдалитьУзел(ОбразецУзлаПоставщик);
			...
			УдалитьУзел(ОбразецУзлаДвижение);
			
		КонецЦикла;
		
		УдалитьУзел(ОбразецУзлаСведПроизвИмпорт);
...
Процедура ЗаполнитьСправочникиДекларации(УзелСправочники, ДанныеВыгрузки)
	...
	УдалитьУзел(ОбразецУзлаПроизводителиИмпортеры);
	...
	УдалитьУзел(ОбразецУзлаПоставщики);
...
Процедура КоманднаяПанельФормыВыгрузитьПакет(Кнопка)
	...
	РезультатВыгрузки = РегламентированнаяОтчетность.КонтекстЭДОЛокальный().ВыгрузитьПакетФСРАР(мСохраненныйДок.Ссылка, Ложь);
...
Процедура КоманднаяПанельФормыОтправить(Кнопка)
	...
	РегламентированнаяОтчетность.КонтекстЭДОЛокальный().ОтправитьОбъектВФСРАР(мСохраненныйДок.Ссылка, Кнопка, Справочники.ВидыОтправляемыхДокументов.РозничнаяПродажаПиваИПивныхНапитков);
...
Процедура КоманднаяПанельФормыПолучитьРезультат(Кнопка)
	...
	Если РегламентированнаяОтчетность.КонтекстЭДОЛокальный().ОбновитьРезультатОтправкиФСРАР(мСохраненныйДок.Ссылка, Справочники.ВидыОтправляемыхДокументов.РозничнаяПродажаПиваИПивныхНапитков) Тогда
...
Процедура КнопкаОбновитьСостояниеОтправкиНажатие(Элемент)
	...
	Если РеквизитыОтправки <> Неопределено Тогда
		Если РеквизитыОтправки.СтатусОтправки = Перечисления.СтатусыОтправки.Отправлен Тогда
			РегламентированнаяОтчетность.КонтекстЭДОЛокальный().ОбновитьРезультатОтправкиФСРАР(мСохраненныйДок.Ссылка, Справочники.ВидыОтправляемыхДокументов.РозничнаяПродажаПиваИПивныхНапитков);
		КонецЕсли;
	КонецЕсли;
...
СтруктураГруппы11.Колонки.Добавить("П000010000101", ОписаниеТиповСтрока);
...
СтруктураГруппы11.Колонки.Добавить("П000010000106", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы11.Колонки.Добавить("П000010000107", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы11.Колонки.Добавить("П000010000108", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы11.Колонки.Добавить("П000010000109", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы11.Колонки.Добавить("П000010000110", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы11.Колонки.Добавить("П000010000111", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы11.Колонки.Добавить("П000010000112", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы11.Колонки.Добавить("П000010000113", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы11.Колонки.Добавить("П000010000114", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы11.Колонки.Добавить("П000010000115", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы11.Колонки.Добавить("П000010000116", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы11.Колонки.Добавить("П000010000117", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы11.Колонки.Добавить("П000010000118", ОписаниеТиповПоказательВДекалитрах);
...
СтруктураГруппы12.Колонки.Добавить("П000010000301", ОписаниеТиповСтрока);
...
СтруктураГруппы12.Колонки.Добавить("П000010000306", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы12.Колонки.Добавить("П000010000307", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы12.Колонки.Добавить("П000010000308", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы12.Колонки.Добавить("П000010000309", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы12.Колонки.Добавить("П000010000310", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы12.Колонки.Добавить("П000010000311", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы12.Колонки.Добавить("П000010000312", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы12.Колонки.Добавить("П000010000313", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы12.Колонки.Добавить("П000010000314", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы12.Колонки.Добавить("П000010000315", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы12.Колонки.Добавить("П000010000316", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы12.Колонки.Добавить("П000010000317", ОписаниеТиповПоказательВДекалитрах);
СтруктураГруппы12.Колонки.Добавить("П000010000318", ОписаниеТиповПоказательВДекалитрах);
...
СтруктураГруппы21.Колонки.Добавить("П000020000101", ОписаниеТиповСтрока);
...
СтруктураГруппы21.Колонки.Добавить("П000020000107", ОписаниеТиповСтрока254);
СтруктураГруппы21.Колонки.Добавить("П000020000108", ОписаниеТиповСтрока254);
СтруктураГруппы21.Колонки.Добавить("П000020000109", ОписаниеТиповДата);
СтруктураГруппы21.Колонки.Добавить("П000020000110", ОписаниеТиповСтрока254);
СтруктураГруппы21.Колонки.Добавить("П000020000111", ОписаниеТиповСтрока254);
СтруктураГруппы21.Колонки.Добавить("П000020000112", ОписаниеТиповПоказательВДекалитрах);
...
СтруктураГруппы22.Колонки.Добавить("П000020000301", ОписаниеТиповСтрока);
...
СтруктураГруппы22.Колонки.Добавить("П000020000307", ОписаниеТиповСтрока254);
СтруктураГруппы22.Колонки.Добавить("П000020000308", ОписаниеТиповСтрока254);
СтруктураГруппы22.Колонки.Добавить("П000020000309", ОписаниеТиповДата);
СтруктураГруппы22.Колонки.Добавить("П000020000310", ОписаниеТиповСтрока254);
СтруктураГруппы22.Колонки.Добавить("П000020000311", ОписаниеТиповСтрока254);
СтруктураГруппы22.Колонки.Добавить("П000020000312", ОписаниеТиповПоказательВДекалитрах);
...
ТаблицаСтраницыРаздел1 = ТаблицаСтраницыРазделаДляМногострочныхРазделов.Скопировать();
