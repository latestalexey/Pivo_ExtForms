...
Процедура ВвестиАдресКонтрагента(Элемент, Область, СтандартнаяОбработка)
	...
	Если ФормаВводаАдреса.ОткрытьМодально() Тогда
		...
		Если ФормаВводаАдреса.ПроизвольныйАдрес Тогда
			...
			Адрес.Адрес = ФормаВводаАдреса.ПредставлениеАдреса;
...
Процедура ПроверитьЗаполнениеДекларация(Параметры, СтатусПроверки)
	...
	Для Каждого ЛистРаздела Из ДанныеРаздела Цикл
		...
		КонецЕсли;
		ПроверитьЗаполнениеТаблицыДекларация(СтатусПроверки, ДанныеДопСтрок, ДополнительныеДанныеЛиста, НомерЛиста);
...
Процедура ПроверитьЗаполнениеТаблицыДекларация(СтатусПроверки, ДанныеДопСтрок, ДополнительныеДанные, НомерЛиста)
	...
	Для Каждого СтрокаДекларации Из ДанныеДопСтрок Цикл
		...
		Иначе
			...
			Если НЕ ПустаяСтрока(КПП) Тогда
				ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации, КПП производителя/импортера должен быть пуст!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателяКПП, НомерЛиста);
			КонецЕсли;
		КонецЕсли;
...
Процедура ПроверитьЗаполнениеВозвраты(Параметры, СтатусПроверки)
	...
	Для Каждого ЛистРаздела Из ДанныеРаздела Цикл
		...
		Если ПустаяСтрока(ДанныеЛиста.ОбособленноеПодразделение) Тогда
			ТекстОписанияОшибки = "На листе " + НомерЛистаСтроковый + " декларации не указаны реквизиты обособленного подразделения!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Возвраты", "ОбособленноеПодразделение", НомерЛиста);
		Иначе
			СведенияОбОП = Неопределено;
			Если ТипЗнч(ЛистРаздела.ДополнительныеДанные) = Тип("Структура") И ЛистРаздела.ДополнительныеДанные.Свойство("СведенияОбОП", СведенияОбОП) Тогда
				ИНН = СокрЛП(СведенияОбОП.ИНН);
		...
		КонецЕсли;
		ПроверитьЗаполнениеТаблицыВозвраты(СтатусПроверки, ДанныеДопСтрок, ДополнительныеДанныеЛиста, НомерЛиста);
...
Процедура ПроверитьЗаполнениеТаблицыВозвраты(СтатусПроверки, ДанныеДопСтрок, ДополнительныеДанные, НомерЛиста)
	...
	Для Каждого СтрокаДекларации Из ДанныеДопСтрок Цикл
		...
		Если ПустаяСтрока(ИНН) Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации, не указан ИНН производителя/импортера!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Возвраты", ИмяПоказателяИНН, НомерЛиста);
		ИначеЕсли (ДлинаИНН <> 10) и (ДлинаИНН <> 12) и (ДлинаИНН <> 9) и (ДлинаИНН <> 8) и (ДлинаИНН <> 14) Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации, неверно указан ИНН производителя/импортера!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Возвраты", ИмяПоказателяИНН, НомерЛиста);
		КонецЕсли;
		...
		Если ДлинаИНН = 10 Тогда
			...
			Если ПустаяСтрока(КПП) Тогда
				ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации, не указан КПП производителя/импортера!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Возвраты", ИмяПоказателяКПП, НомерЛиста);
			ИначеЕсли СтрДлина(КПП) <> 9 Тогда
				ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации, неверно указан КПП производителя/импортера!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Возвраты", ИмяПоказателяКПП, НомерЛиста);
			КонецЕсли;
...
Функция СформироватьСтруктуруПараметров()
	...
	Параметры.Вставить("ИдФайл", ИдФайл(Параметры));
...
Процедура ЗаполнитьАвтоБухгалтерия(СписокОрганизаций) Экспорт
	...
	Состояние("Выполнение запроса для деклараций");
	...
	Сч=1; Колво=ТаблицаЗапроса.Количество();
	Для Каждого СтрокаЗапроса Из ТаблицаЗапроса Цикл
		Состояние("Заполнение декларации",Сч/Колво*100);
		...
		СтрокаЗаполнения.П000010000118=ОбщегоНазначения.ПолучитьНомерНаПечать(Стр);
		...
		КонецЕсли;
		СтрокаЗаполнения.П000010000115=ПривестиНомерНаПечать(СтрокаЗапроса.П000010000115);
	...
	ДополнительныеДанныеСтраницы = АктивнаяСтраница.ДополнительныеДанные;
	Состояние("Выполнение запроса для возвратов");
	ТаблицаЗапроса = Запрос.Выполнить().Выгрузить();
	Состояние("Запрос выволнен");
	...
	Состояние("Начало выполнения расчета");
	Расчет();
	Состояние("");
КонецПроцедуры
