...
Процедура ЗаполнитьАвтоБухгалтерияКОРП(СписокОрганизаций)
	...
	Для Каждого СтрокаЗапроса Из ТаблицаЗапроса Цикл
		...
		//KAV 
		СтрокаЗаполнения.П000020000110=ПолучитьНомерНаПечать(СтрокаЗаполнения.П000020000110);
		НС = НС + 1;
	КонецЦикла;
	...
	Для Каждого Подразделение Из ДеревоПоПодразделениям.Строки Цикл
		...
		Для Каждого СтрокаПодразделения Из Подразделение.Строки Цикл
			...
			ЗаполнитьЗначенияСвойств(СтрокаЗаполнения, СтрокаПодразделения);
			СтрокаЗаполнения.П000020000310=ПолучитьНомерНаПечать(СтрокаЗаполнения.П000020000310);
			НС = НС + 1;
		КонецЦикла;
		ИзменитьТаблицуПараметровАвтозаполнения(1, "Раздел2");
		Если ВыводНаПервуюСтраницу Тогда
...
Процедура ПроверитьЗаполнениеРаздел1(Параметры, СтатусПроверки)
	...
	Для Каждого ЛистРаздела Из ДанныеРаздела Цикл
		...
		Иначе
			...
			Если ТипЗнч(ЛистРаздела.ДополнительныеДанные) = Тип("Структура") И ЛистРаздела.ДополнительныеДанные.Свойство("СведенияОбОП", СведенияОбОП) Тогда
				...
				Если ПустаяСтрока(ИНН) Тогда
					...
					СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", "ОбособленноеПодразделение", НомерЛиста);
				ИначеЕсли НЕ Параметры.ЭтоПБОЮЛ И СтрДлина(ИНН) <> 10 Тогда
					ТекстОписанияОшибки = "На листе " + НомерЛистаСтроковый + " декларации неверно указан ИНН обособленного подразделения!";
					СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", "ОбособленноеПодразделение", НомерЛиста);
				ИначеЕсли Параметры.ЭтоПБОЮЛ И СтрДлина(ИНН) <> 12 Тогда
					ТекстОписанияОшибки = "На листе " + НомерЛистаСтроковый + " декларации неверно указан ИНН индивидуального предпринимателя!";
					СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", "ОбособленноеПодразделение", НомерЛиста);
				КонецЕсли;
		...
		Если ТипЗнч(ЛистРаздела.ДополнительныеДанные) = Тип("Структура") Тогда
			ЛистРаздела.ДополнительныеДанные.Свойство(мИдГруппы11, ДополнительныеДанныеЛиста);
		КонецЕсли;
		ПроверитьЗаполнениеТаблицыРаздел1(СтатусПроверки, ДанныеДопСтрок, ДополнительныеДанныеЛиста, НомерЛиста);
		
		ЛистРаздела.ДанныеДопСтрок.Свойство(мИдГруппы12, ДанныеДопСтрок);
		Если ТипЗнч(ЛистРаздела.ДополнительныеДанные) = Тип("Структура") Тогда
			ЛистРаздела.ДополнительныеДанные.Свойство(мИдГруппы12, ДополнительныеДанныеЛиста);
		КонецЕсли;
		ПроверитьЗаполнениеТаблицыРаздел1(СтатусПроверки, ДанныеДопСтрок, ДополнительныеДанныеЛиста, НомерЛиста);
...
Процедура ПроверитьЗаполнениеТаблицыРаздел1(СтатусПроверки, ДанныеДопСтрок, ДополнительныеДанные, НомерЛиста)
	...
	ТаблицаДубликатов = ДанныеДопСтрок.Скопировать();
	...
	ТаблицаДубликатов.Колонки.Добавить("Счетчик", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(10));
...
Процедура ПроверитьЗаполнениеРаздел2(Параметры, СтатусПроверки)
	...
	Для Каждого ЛистРаздела Из ДанныеРаздела Цикл
		...
		Если ТипЗнч(ЛистРаздела.ДополнительныеДанные) = Тип("Структура") Тогда
			ЛистРаздела.ДополнительныеДанные.Свойство(мИдГруппы21, ДополнительныеДанныеЛиста);
		КонецЕсли;
		ПроверитьЗаполнениеТаблицыРаздел2(СтатусПроверки, ДанныеДопСтрок, ДополнительныеДанныеЛиста, НомерЛиста);
		
		ЛистРаздела.ДанныеДопСтрок.Свойство(мИдГруппы22, ДанныеДопСтрок);
		Если ТипЗнч(ЛистРаздела.ДополнительныеДанные) = Тип("Структура") Тогда
			ЛистРаздела.ДополнительныеДанные.Свойство(мИдГруппы22, ДополнительныеДанныеЛиста);
		КонецЕсли;
		ПроверитьЗаполнениеТаблицыРаздел2(СтатусПроверки, ДанныеДопСтрок, ДополнительныеДанныеЛиста, НомерЛиста);
...
Процедура ПроверитьЗаполнениеТаблицыРаздел2(СтатусПроверки, ДанныеДопСтрок, ДополнительныеДанные, НомерЛиста)
	...
	Для Каждого СтрокаДекларации Из ДанныеДопСтрок Цикл
		...
		ИначеЕсли СтрДлина(ИНН) <> ДлинаИНН И СтрДлина(ИНН) <> 9 И СтрДлина(ИНН) <> 8 Тогда // Длина ИНН = 9 для УНП Беларусь, 8 - Армения.
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации неверно указан ИНН производителя/импортера!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателяИНН, НомерЛиста);
		КонецЕсли;
		...
		Если ПустаяСтрока(ИНН) Тогда
			...
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателяИНН, НомерЛиста);
		ИначеЕсли СтрДлина(ИНН) <> 10 И СтрДлина(ИНН) <> 9 И СтрДлина(ИНН) <> 8 И СтрДлина(ИНН) <> 12 Тогда
		...
		Иначе
			Если НЕ ПустаяСтрока(КПП) Тогда
				...
				ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации указан КПП у поставщика, не являющегося юридическим лицом, зарегистрированным на территории Российской федерации!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателяКПП, НомерЛиста);
			КонецЕсли;
		КонецЕсли;
