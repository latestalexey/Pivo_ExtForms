...
Процедура СоздатьПТУ(РТУ)
	...
	Для Каждого СтрРТУ Из РТУ.Товары Цикл
		...
		//ОбработкаТабличныхЧастей.ПриИзмененииНоменклатурыТабЧасти(СтрокаТЧ, ПТУ);
		//ОбработкаТабличныхЧастей.ЗаполнитьНомерГТДИСтрануПроисхождения(СтрокаТЧ, Ложь, ПТУ.Организация, ПТУ.Дата);
		ПТУ.ЗаполнитьСчетаУчетаВСтрокеТабЧастиРегл(СтрокаТЧ, "Товары", Истина, Истина);
		// Заполняем реквизиты табличной части.
		//ОбработкаТабличныхЧастей.ЗаполнитьСтавкуНДСТабЧасти(СтрокаТЧ, ПТУ);
		// Заполнение цены
		//ОбработкаТабличныхЧастей.ЗаполнитьЕдиницуЦенуПродажиТабЧасти(СтрокаТЧ, ПТУ, Константы.ВалютаРегламентированногоУчета.Получить());
		// Рассчитать реквизиты табличной части.
		//ОбработкаТабличныхЧастей.РассчитатьСуммуТабЧасти(СтрокаТЧ, ПТУ);
		//ОбработкаТабличныхЧастей.РассчитатьСуммуНДСТабЧасти(СтрокаТЧ, ПТУ);
		СтрокаТЧ.СерияНоменклатуры=МодульВнутреннихПродаж.ПолучитьСерию(СтрокаТЧ.Номенклатура,Поставщик,Покупатель,НачалоДня(ПТУ.Дата),СтрокаТЧ.СерияНоменклатуры.Код);
