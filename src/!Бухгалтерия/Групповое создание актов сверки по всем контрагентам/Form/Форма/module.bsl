...
Процедура ПриОткрытии()
	Построитель.Текст="ВЫБРАТЬ РАЗЛИЧНЫЕ
	                  |	ВложенныйЗапрос.Организация КАК Организация,
	                  |	ВложенныйЗапрос.Контрагент,
	                  |	ВложенныйЗапрос.ИмяКонтрагента КАК ИмяКонтрагента
	                  |ИЗ
	                  |	(ВЫБРАТЬ
	                  |		ХозрасчетныйОбороты.Организация КАК Организация,
	                  |		ХозрасчетныйОбороты.Субконто1 КАК Контрагент,
	                  |		ХозрасчетныйОбороты.Субконто1.Наименование КАК ИмяКонтрагента
	                  |	ИЗ
	                  |		РегистрБухгалтерии.Хозрасчетный.Обороты(&ДатаНачала, &ДатаКонца, , Счет В ИЕРАРХИИ (&СпСчетов), &СпВидов, {(Субконто1).* КАК Контрагент,(Субконто2).* КАК Договор, (Организация).* КАК Организация}, , ) КАК ХозрасчетныйОбороты
	                  |	
	                  |	ОБЪЕДИНИТЬ ВСЕ
	                  |	
	                  |	ВЫБРАТЬ
	                  ...
	                  |	ИЗ
	                  |		РегистрБухгалтерии.Хозрасчетный.Остатки(&ДатаНачала, Счет В ИЕРАРХИИ (&СпСчетов), &СпВидов, {(Субконто1).* КАК Контрагент, (Субконто2).* КАК Договор,(Организация).* КАК Организация}) КАК ХозрасчетныйОстатки) КАК ВложенныйЗапрос
	                  |
	                  |УПОРЯДОЧИТЬ ПО
	                  |	ИмяКонтрагента
	                  |ИТОГИ ПО
	                  |	Организация";
	СпВидов=Новый Массив;
	СпВидов.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты);
	СпВидов.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Договоры);
	Построитель.Параметры.Вставить("СпВидов",СпВидов);
...
Процедура СоздатьАктыНажатие(Элемент)
	...
	Построитель.Параметры.Вставить("СпСчетов", СписокСчетов);
	...
	Пока ВыборкаПоОрганизациям.Следующий() Цикл
		...
		Пока ВыборкаПоКонтрагентам.Следующий() Цикл
			...
			//Док.Комментарий		= "Создан Тестовой обработкой для проверки "+ЭтотОбъект.Метаданные().Представление()+" "+Формат(ТекущаяДата(),"ДЛФ=DD");
			Док.Дата			= ?(ЗначениеЗаполнено(ДатаФормирования),ДатаФормирования, НачалоДня(ТекущаяДата()));
			...
			Док.ЗаполнитьПоДаннымБухгалтерскогоУчета();
...
Процедура ОсновныеДействияФормыСохранитьВПапку(Кнопка)
	...
	ПостроительДоки = Новый ПостроительОтчета;
	ПостроительДоки.Текст = 
		"ВЫБРАТЬ
		...
		|ГДЕ
		|	АктСверкиВзаиморасчетов.Комментарий ПОДОБНО &Комментарий
	...
	ПостроительДоки.Параметры.Вставить("ДатаОкончания", КонецДня(КонПериода));
	ПостроительДоки.Параметры.Вставить("ДатаНачала", НачалоДня(НачПериода));
	ПостроительДоки.Параметры.Вставить("Комментарий", "Создан обработкой%");
	ПостроительДоки.Выполнить();
	
	ПостроительДоки.УстановитьНастройки(Построитель.ПолучитьНастройки());

	Результат = ПостроительДоки.ПолучитьЗапрос().Выполнить();

	ВыборкаДетальныеЗаписи = Результат.Выбрать();

	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ТабДок=Печать(ВыборкаДетальныеЗаписи.Ссылка);
		Попытка
			ТабДок.Записать(Диалог.Каталог+"\АктСверки_"+УбратьЗапрещенныеЗнаки(ВыборкаДетальныеЗаписи.Контрагент.Наименование)+"("+СтрЗаменить(ПредставлениеПериода(НачалоДня(НачПериода),КонецДня(КонПериода)),".","-")+ ").pdf",ТипФайлаТабличногоДокумента.PDF);
		Исключение
...
Функция Печать(СсылкаНаОбъект) Экспорт
	...
	ОбластьПодвал.Рисунки.Печать.Картинка=СсылкаНаОбъект.Организация.Печать.Получить();
	ОбластьПодвал.Рисунки.Подпись.Картинка=СсылкаНаОбъект.Организация.Подпись.Получить();
