...
Процедура СоздатьТТН_ИсхДляЕГАИС()
	...
	Для каждого Стр_РеализацияТМЦ из РеализацииТМЦ Цикл
		...
		////Было **КМН***
		////Если ТипЗнч(Стр_РеализацияТМЦ.РеализацияТМЦ)=Тип("ДокументСсылка.РеализацияТоваровУслуг") и Стр_РеализацияТМЦ.РеализацияТМЦ.Контрагент.ЮрФизЛицо=Перечисления.ЮрФизЛицо.ЮрЛицо Тогда
		//Если ТипЗнч(Стр_РеализацияТМЦ.РеализацияТМЦ)<>Тип("ДокументСсылка.ПеремещениеТоваров") и Стр_РеализацияТМЦ.РеализацияТМЦ.Контрагент.ЮрФизЛицо=Перечисления.ЮрФизЛицо.ЮрЛицо Тогда
		//	Контрагент=Стр_РеализацияТМЦ.РеализацияТМЦ.Грузополучатель;
		//	ПараметрыКонтрагентаДляЕГАИС                       = ПолучитьПараметрыЕГАИСДляКонтрагента(Контрагент, Ложь);
		//	Если ПараметрыКонтрагентаДляЕГАИС=Неопределено Тогда 
		//		Сообщить("Не задан контрагент ЕГАИС для "+Строка(Контрагент)+". Будет использован головной контрагент");
		//		
		//		Контрагент=Стр_РеализацияТМЦ.РеализацияТМЦ.Контрагент;
		//		ПараметрыКонтрагентаДляЕГАИС                   = ПолучитьПараметрыЕГАИСДляКонтрагента(Контрагент, Ложь);
		//	КонецЕсли;
		////***КМН*** кто-то нагадил 29092016 с 17-00 до 18-00 ....
		//// не было учтено, что у перемещения нету ГРУЗОПОЛУЧАТЕЛЯ
		////Иначе
		////	Контрагент=Стр_РеализацияТМЦ.РеализацияТМЦ.Грузополучатель;
		////	ПараметрыКонтрагентаДляЕГАИС                       = ПолучитьПараметрыЕГАИСДляКонтрагента(Контрагент, Ложь);
		//КонецЕсли;
		...
		//KAV
		//Проверяем только один случай - реализация от юр лица
		Если ТипЗнч(Стр_РеализацияТМЦ.РеализацияТМЦ)=Тип("ДокументСсылка.РеализацияТоваровУслуг") и Стр_РеализацияТМЦ.РеализацияТМЦ.Контрагент.ЮрФизЛицо=Перечисления.ЮрФизЛицо.ЮрЛицо Тогда
			Контрагент=Стр_РеализацияТМЦ.РеализацияТМЦ.Грузополучатель;
			ПараметрыКонтрагентаДляЕГАИС                       = ПолучитьПараметрыЕГАИСДляКонтрагента(Контрагент, Ложь);
			Если ПараметрыКонтрагентаДляЕГАИС=Неопределено Тогда 
...
<<<<<<< HEAD
			...
			ПараметрыКонтрагентаДляЕГАИС                       = ПолучитьПараметрыЕГАИСДляКонтрагента(Контрагент, Ложь);
=======
		//Во всех остальных случаях - из контрагента
			Контрагент=Стр_РеализацияТМЦ.РеализацияТМЦ.Контрагент;
			ПараметрыКонтрагентаДляЕГАИС                   = ПолучитьПараметрыЕГАИСДляКонтрагента(Контрагент, Ложь);
		КонецЕсли;
		
		Если ПараметрыКонтрагентаДляЕГАИС=Неопределено Тогда 
			Сообщить("Не задан контрагент ЕГАИС для "+Строка(Контрагент)+". Документ создан не будет");
			Продолжить; 
>>>>>>> feature/Формирование_документов_для_ЕГАИС
		КонецЕсли;
...
Процедура Т_ТТН_Прих_ВУчСистемуПоступлениеТМЦНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	Если ЭлементыФормы.Т_ТТН_Прих_ВУчСистему.ТекущиеДанные.ТТН_Прих.ТипОперации=Перечисления.ЕГАИС_ТипыОпераций.ВозвратОтПокупателя Тогда
		...
		Если Рез.Значение="Возврат товаров от покупателя" Тогда
		...
		Иначе
			...
			Форма.ДокументСписок.Отбор.ЭтоВозврат.Установить(Истина);
		КонецЕсли;
	Иначе
		//Доп выбор
		СписокВыбора=Новый СписокЗначений;
		...
		Если Рез.Значение="Поступление товаров" Тогда
			...
			Если ЗначениеЗаполнено(ОбособленноеПодразделение) Тогда
				Форма.ДокументСписок.Отбор.ПодразделениеОрганизации.Установить(ОбособленноеПодразделение);
			Иначе
				//***КМН*** выключить отбор когда не надо 28092016 Заявка Белокон Ольги
				Ответ = Вопрос("Установить отбор по подразделениям?",РежимДиалогаВопрос.ДаНет,15,,"Вопрос",);
		...
		ИначеЕсли Рез.Значение="Перемещение товаров" Тогда
			...
			Форма.ДокументСписок.Отбор.Контрагент.ВидСравнения=ВидСравнения.НеРавно;
			Форма.ДокументСписок.Отбор.Контрагент.Использование=Истина;
			Форма.ДокументСписок.Отбор.ЭтоВозврат.Установить(Ложь);
