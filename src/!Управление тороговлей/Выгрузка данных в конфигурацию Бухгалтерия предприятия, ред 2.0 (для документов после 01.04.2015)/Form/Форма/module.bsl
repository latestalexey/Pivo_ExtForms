...
Процедура ВыборФайла(Элемент, ПроверятьСуществование=Ложь)
	...
	РаботаСФайлами.ВыбратьКаталог(КаталогВыгрузки);
...
Процедура КнопкаВыполнитьНажатие(Элемент)
	Если СпОрганизаций.Количество()=0 Тогда
		Предупреждение("Не выбраны организации");
		Возврат;
	КонецЕсли;
	Если ПустаяСтрока(КаталогВыгрузки) Тогда
		Предупреждение("Не указан каталог для выгрузки");
		Возврат;
	КонецЕсли;
	...
	КонецЕсли;
   	ЭлементыФормы.ИндикаторВыгрузки.МаксимальноеЗначение=СпОрганизаций.Количество();
	ИндикаторВыгрузки=0;
	Для Каждого ВыбОрганизация из СпОрганизаций Цикл
		Организация=ВыбОрганизация.Значение;
		ФайлВыгрузки=КаталогВыгрузки+"/"+Организация.Префикс+".xml";
		ВыполнитьВыгрузку();
		ИндикаторВыгрузки=ИндикаторВыгрузки+1;
	КонецЦикла;
...
//	мУниверсальнаяВыгрузкаДанных.ФлагРежимОтладкиОбработчиков = Истина;;
	...
	//KAV
	мУниверсальнаяВыгрузкаДанных.Параметры.Вставить("СкрытьКодНоменклатуры",			СкрытьКодНоменклатуры);
	//KAV
