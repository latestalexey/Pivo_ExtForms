...
Процедура КнопкаВыполнитьНажатие(Кнопка)
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		КонецЕсли;
		СтрокаТовары=ДокВозврат.ВозвратнаяТара.Добавить();
		...
		СтрокаТовары=ДокРеализация.ВозвратнаяТара.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаТовары,СтрокаТЧ);
...
Процедура КоманднаяПанель1Заполнить(Кнопка)
	...
	Запрос.Текст = 
		...
		|				И Номенклатура В ИЕРАРХИИ(&Кеги)) КАК ТоварыПереданныеОстаткиИОбороты
		|
		|УПОРЯДОЧИТЬ ПО
