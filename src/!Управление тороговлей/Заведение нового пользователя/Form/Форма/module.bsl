...
#Область Копируем_доп_настройки
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЗначенияДополнительныхПравПользователя.Пользователь,
...
#Область Копируем_группы_пользователей
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ГруппыПользователей.Ссылка
...
Процедура ЗаблокироватьНажатие(Элемент)
	//На всякий случай.
	Ответ=Вопрос("Вы уверены что хотите заблокировать пользователя?",РежимДиалогаВопрос.ДаНет,30,КодВозвратаДиалога.Нет);
	Если Ответ=КодВозвратаДиалога.Да Тогда
		//1. Блокировка пользователя ИБ
		ПользовательИБ=УправлениеПользователями.НайтиПользователяИБ(СокрЛП(ПользовательСмерть.Код));
		Если ПользовательИБ<>Неопределено Тогда
			ПользовательИБ.АутентификацияСтандартная=Ложь;
			ПользовательИБ.ПоказыватьВСпискеВыбора=Ложь;
			ПользовательИБ.Имя="*"+ПользовательИБ.Имя;
			ПользовательИБ.ПолноеИмя="*"+ПользовательИБ.ПолноеИмя;
			ПользовательИБ.Записать();
		КонецЕсли;
		Об=ПользовательСмерть.ПолучитьОбъект();
		Об.Код="*"+Об.Код;
		Об.Наименование="*"+Об.Наименование;
		Об.Записать();
