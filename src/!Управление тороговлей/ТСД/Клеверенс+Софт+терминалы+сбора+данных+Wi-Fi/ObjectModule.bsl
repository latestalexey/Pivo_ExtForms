...
Функция ПолучитьОписание() Экспорт
	...
	Модели.Добавить("CipherLab 9500 (Win CE)");
	Модели.Добавить("CipherLab 9600");
	Модели.Добавить("CipherLab 9671");
	Модели.Добавить("CipherLab 9700 (Win CE)");
	...
	Модели.Добавить("MobileBase DS5 (Mobile)");
	...
	Если Ответ Тогда
		Для Каждого СтрокаТаблицы Из ТаблицаНовыхШтрихкодов Цикл
			...
			Если Товар <> Неопределено Тогда
				...
				ШК = СтрокаТаблицы.Штрихкод;
				ШК1 = ШК;
				Пока СтрДлина(ШК1) < 13 Цикл
					ШК1 = "0" + ШК1;									
				КонецЦикла;
				
				Если РаботаСТорговымОборудованием.ТипШтрихкодаEAN13ТО(ШК1) Тогда					
					Запрос.УстановитьПараметр("Штрихкод", ШК1);
					РезультатЗапроса = Запрос.Выполнить();
					...
					//	ТаблицаРез = РезультатЗапроса.Выгрузить();
					//	ДобавленныеШК.Добавить(СтрокаТаблицы.Штрихкод);
					//	Сообщить("Штрихкод не добавлен! Штрихкод: '" + ШК + "' уже заведен для товара: '" + ТаблицаРез[0].Владелец + "'.");				
					//	Продолжить;	
					//КонецЕсли;		
				КонецЕсли;
				...
				Если ХарактеристикаИмя <> Неопределено И Не ПустаяСтрока(ХарактеристикаИмя) Тогда
				...
				Попытка
					...
					ДобавленныеШК.Добавить(СтрокаТаблицы.Штрихкод);
				Исключение
					Сообщить("Для товара '" + Товар.Наименование + "' не удалось добавить штрихкод '" + ШК + "'. Ошибка: " + ОписаниеОшибки(), СтатусСообщения.ОченьВажное);	
					Продолжить;	
				КонецПопытки;
				
				Сообщ = "Для товара '" + Товар.Наименование + "' добавлен штрихкод '" + ШК + "'";
				Если Не ПустаяСтрока(Строка(Запись.ХарактеристикаНоменклатуры)) Тогда
				   Сообщ = Сообщ + " Характеристика: " + Запись.ХарактеристикаНоменклатуры;
				КонецЕсли;   
				
				Если Не ПустаяСтрока(Строка(Запись.СерияНоменклатуры)) Тогда
				   Сообщ = Сообщ + " Серия: " + Запись.СерияНоменклатуры;
				КонецЕсли;
...
Функция Версия_3_3нака_СтрокаВЧисло(сВерсия) Экспорт
	нВерсия 	  = 0;
...
Функция ПолучитьСтрокуПодключения(ИпАдрес) Экспорт
	...
	Если ПустаяСтрока(Рез) Тогда
		Рез = ПолучитьIPПоУмолчанию()+":9400";
	КонецЕсли;
	
	Если Найти(Рез, ":") = 0 Тогда
		Рез = Рез + ":9400";
	КонецЕсли;
	
	Возврат "http://" + Рез + "/MobileSMARTS/DataStorage.asmx";	
КонецФункции
