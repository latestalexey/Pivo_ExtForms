...
Функция ИнициализацияИтоговойТаблицы() Экспорт
	...
	Таблица.Колонки.Добавить("Количество",			ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15,3));
	Возврат Таблица;
КонецФункции

Функция ИнициализацияТаблицыРазбораДокумента() Экспорт
	...
	ТаблицаОригинальныхВерсий.Колонки.Добавить("ГруппировкаВерсий",					ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(10,0));
...
#Область Инициализация_таблицы
	ТаблицаОригинальныхВерсий=ИнициализацияТаблицыРазбораДокумента();
#КонецОбласти
	...
	Пока Версии.Следующий() Цикл
		...
		ЗнПричины=Док.Реквизиты.Найти("ПричинаРедактированияДокумента");
		Если ЗнПричины=Неопределено Тогда
			Причина		= Справочники.ПричиныИсправленияДокумента.ПустаяСсылка();
		Иначе
			Причина		= Справочники.ПричиныИсправленияДокумента.ПолучитьСсылку(Новый УникальныйИдентификатор(ЗнПричины.ЗначениеРеквизита));
		КонецЕсли;
		...
		Пока Сч < Оригинал.Количество() Цикл
			...
			СтрокаВерсий.ГруппировкаВерсий=СчГруппировок;
			Сч=Сч+1;
		КонецЦикла;
		СчГруппировок=СчГруппировок+1;
	КонецЦикла;
	Состояние("");
	Возврат ТаблицаОригинальныхВерсий;
КонецФункции

Процедура ВывестиРезультат(ТаблицаОригиналов,ТаблицаВерсий,ПолеДляВывода,ОтборНоменклатуры) Экспорт
	ПолеДляВывода.Очистить();
	ВнешниеНаборыДанных=Новый Структура("ТаблицаОригиналов,ТаблицаВерсий",ТаблицаОригиналов,ТаблицаВерсий);
	...
	Отборы=Новый Массив;
	Для Каждого Эл Из ОтборНоменклатуры Цикл
		Если НЕ Эл.Использование Тогда Продолжить; КонецЕсли;
		ЭлементОтбора1 = КомпоновщикНастроек.Настройки.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ЭлементОтбора1.ЛевоеЗначение = КомпоновщикНастроек.Настройки.ДоступныеПоляОтбора.НайтиПоле(Новый ПолеКомпоновкиДанных("НоменклатураОригинал")).Поле;
	
		Если Эл.ВидСравнения=ВидСравнения.Равно Тогда
			ЭлементОтбора1.ВидСравнения=ВидСравненияКомпоновкиДанных.Равно;
		ИначеЕсли Эл.ВидСравнения=ВидСравнения.НеРавно Тогда
			ЭлементОтбора1.ВидСравнения=ВидСравненияКомпоновкиДанных.НеРавно;
		ИначеЕсли Эл.ВидСравнения=ВидСравнения.ВИерархии Тогда
			ЭлементОтбора1.ВидСравнения=ВидСравненияКомпоновкиДанных.ВИерархии;
		ИначеЕсли Эл.ВидСравнения=ВидСравнения.НеВИерархии Тогда
			ЭлементОтбора1.ВидСравнения=ВидСравненияКомпоновкиДанных.НеВИерархии;
		ИначеЕсли Эл.ВидСравнения=ВидСравнения.ВСписке Тогда
			ЭлементОтбора1.ВидСравнения=ВидСравненияКомпоновкиДанных.ВСписке;
		ИначеЕсли Эл.ВидСравнения=ВидСравнения.НеВСписке Тогда
			ЭлементОтбора1.ВидСравнения=ВидСравненияКомпоновкиДанных.НеВСписке;
		ИначеЕсли Эл.ВидСравнения=ВидСравнения.ВСпискеПоИерархии Тогда
			ЭлементОтбора1.ВидСравнения=ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии;
		Иначе
			ЭлементОтбора1.ВидСравнения=ВидСравненияКомпоновкиДанных.Равно;
		КонецЕсли;
		ЭлементОтбора1.ПравоеЗначение=Эл.Значение;
		Отборы.Добавить(ЭлементОтбора1);
	КонецЦикла;
	...
	ПолеДляВывода.ПоказатьУровеньГруппировокСтрок(1);
КонецПроцедуры
