...
Перем мВалютаРегламентированноеУчета;
...
#КонецОбласти
	...
	Для Каждого СтрокаДляОбработки Из ТаблицаТоваров Цикл
		...
		Если КолвоТемп<>0 Тогда
			...
			ДопОстатки.ТипОрганизации	= СхемаДляДозакупаОстатков;
				...
				Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
					Если ФлагРеал Тогда
						...
						Иначе
							...
							ОбРеал.ВалютаДокумента, ЗаполнениеДокументов.КурсДокумента(ОбРеал, мВалютаРегламентированноеУчета), 
							ЗаполнениеДокументов.КратностьДокумента(ОбРеал, мВалютаРегламентированноеУчета));
							Если Не ЗначениеЗаполнено(Цена) Тогда
								...
								Если РассчитанныеЦены<>Неопределено Тогда
									Если РассчитанныеЦены.ВновьРассчитаннаяФиксированнаяЦенаВП>РассчитанныеЦены.ФиксированнаяЦенаВП*1.05 Тогда//сюда же включается Фикс=0
										...
										СтрЗаписи.Валюта			= мВалютаРегламентированноеУчета;
					...
					Если ФлагПост Тогда
						...
						Если НЕ ФлагРеал Тогда //Если Была реализация то цена уже расчитана
							...
							Иначе
								...
								ОбПост.ВалютаДокумента, ЗаполнениеДокументов.КурсДокумента(ОбПост, мВалютаРегламентированноеУчета), 
								ЗаполнениеДокументов.КратностьДокумента(ОбПост, мВалютаРегламентированноеУчета));
								Если НЕ ЗначениеЗаполнено(Цена) Тогда
...
Функция СоздатьРеализацию(Договор,Склад,ДатаДокумента,Розлив)
	...
	ДокРеализации.ВалютаДокумента=мВалютаРегламентированноеУчета;
...
Функция СоздатьПоступление(Договор,Склад,ДатаДокумента,НомерВнутр=Неопределено,ДатаВнутр,Розлив)
	...
	ДокПоступления.ВалютаДокумента=мВалютаРегламентированноеУчета;
...
Функция СоздатьВозвратПоставщику(Перемещение)
	...
	Об.ВалютаДокумента=мВалютаРегламентированноеУчета;
	...
	Если Не ЗначениеЗаполнено(Об.ДоговорКонтрагента) Тогда 
		...
		НовыйДоговор.ВалютаВзаиморасчетов=мВалютаРегламентированноеУчета;
...
мВалютаРегламентированноеУчета=мВалютаРегламентированноеУчета;
